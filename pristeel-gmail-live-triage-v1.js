...
function ensureFollowupRender(){
  var root=document.getElementById('pst-ws-inbox-emails');if(!root||root.querySelector('.pst-ofu-toolbar'))return;
  var buttons=root.querySelectorAll('.pst-ws-rowaction'),legacy=false;
  for(var i=0;i<buttons.length;i++){if(text(buttons[i].textContent)==='Lidhe'){legacy=true;break;}}
  if(!legacy)return;
  var F=window.PSTOutreachFollowupV1;if(!F||typeof F.refresh!=='function')return;
  var fs=F.state;if(fs&&fs.loading)return;
  F.refresh();
}
async function refresh(){if(state.busy)return;var G=window.PSTGmailLiveInboxV2,rows=G&&G._state&&G._state.rows||[];if(!rows.length){restoreHeadings();await injectWaiting();ensureFollowupRender();return;}state.busy=true;var seq=++state.seq;try{await loadContext(rows,seq);if(seq!==state.seq)return;applyRows();await injectWaiting();ensureFollowupRender();}finally{if(seq===state.seq)state.busy=false;}}
