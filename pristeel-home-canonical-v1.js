/* PRISTEEL Home Canonical v2
 * Single data owner for Workspace Home.
 * Action engine: project events -> current state -> next best action -> concise Home priorities.
 * Keeps all outbound/commercial decisions human-gated.
 */
(function(){
'use strict';
...
/* Legacy Home writers stay retired. */
...
function ageDays(v){var d=safeDate(v);return d?Math.max(0,Math.floor((Date.now()-d.getTime())/86400000)):null;}
...
function stageName(id){return({rfq_in:'Kërkesa e klientit',technical_review:'Verifikimi teknik',supplier_selection:'Zgjedhja e prodhuesit',pricing:'Përcaktimi i çmimit',client_offer:'Oferta & konfirmimi',commercial:'Përpunimi komercial',production_control:'Koordinimi i prodhimit',factory_audit:'Auditimi i uzinës',transport:'Transporti & dërgesa'})[String(id||'')]||'Në pritje';}
...
function sentTimeForOffer(ctx,d){
 if(!d)return 0;var nr=flat(docNr(d)),created=ms(d.created_at),best=0;
...
function latestOutgoing(ctx){return latest(arr(ctx.emails).filter(isOutgoing),'sent_at');}
...
function extractExecutionDeadline(text){
 text=String(text||'');var m=text.match(/(?:najkasnije\s+do|najkasnije|rok(?:\s+realizacije)?[^\d]{0,30}|zavr(?:s|š)ene[^\d]{0,30})(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{4})/i);if(!m)return'';
 var d=new Date(Number(m[3]),Number(m[2])-1,Number(m[1]));if(isNaN(d.getTime()))return'';return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
}
...
function supplierQuoteReady(ctx){
 ...
 var body=flat((reply.subject||'')+' '+(reply.snippet||''));var rfqState=norm(rfqMatch.status);
...
function currentState(ctx){
 ...
 var p=ctx.project||{};if(relDays(p.deadline)!==null&&relDays(p.deadline)<=3&&norm(p.pipeline_stage)==='technical_review')return'Sqarime teknike para ofertës';
 return stageName(p.pipeline_stage);
}
function taskScore(t){
 var d=relDays(t&&t.due_date);if(norm(t&&t.source)==='email_request_auto')return 780;
 if(d===null)return 0;if(d<0)return 700+Math.min(120,Math.abs(d));if(d===0)return 690;if(d<=3)return 650-d;return 0;
}
...
function actionSourceEmail(ctx,a){if(a&&a.email)return a.email;var e=latestRelevantEmail(ctx);return e||null;}
function buildActions(data,contexts){
 var candidates=[];
 Object.keys(contexts).forEach(function(pid){
  var ctx=contexts[pid],p=ctx.project;if(!p||inactiveProject(p))return;
 ...
 arr(data.rfqs).forEach(function(r){
  var pid=String(r.project_id||''),ctx=contexts[pid],p=ctx&&ctx.project;if(p&&inactiveProject(p))return;var st=norm(r.status);if(['replied','won','lost','planned'].indexOf(st)>-1)return;
...
function buildWaiting(contexts){var out=[];Object.keys(contexts).forEach(function(pid){var ctx=contexts[pid],p=ctx.project;if(!p||inactiveProject(p))return;var w=waitingAfterOffer(ctx);if(!w)return;var nr=docNr(w.offer)||'Oferta jonë';var dl=p.deadline?(' · realizimi '+dateText(p.deadline)):'';out.push({project_id:pid,name:p.name||'Projekt',client:p.client||'klientit',text:nr+' u dërgua '+dateText(w.sent)+dl,activity:w.sent,kind:'client'});});return out.sort(function(a,b){return b.activity-a.activity;}).slice(0,4);}
function nextForProject(ctx,actions){if(!ctx)return'Në pritje';var pid=String(ctx.project&&ctx.project.id||'');var a=arr(actions).filter(function(x){return x.project_id===pid;})[0];if(a)return a.title;var w=waitingAfterOffer(ctx);if(w)return'Në pritje të përgjigjes së klientit';var sq=supplierQuoteReady(ctx);if(sq)return'Përgatit ofertën PRISTEEL';var t=latestOpenTask(ctx);if(t&&!taskIsSuperseded(ctx,t))return specificTaskTitle(ctx,t);var an=latestAnalysis(ctx.analyses);return one(recommendation(an),stageName(ctx.project&&ctx.project.pipeline_stage));}
function deriveProjects(data,contexts,actions){return arr(data.projects).filter(function(p){return !inactiveProject(p);}).map(function(p){var id=String(p.id||''),ctx=contexts[id]||{project:p};var e=latest(ctx.emails,'sent_at'),t=latest(ctx.tasks,'created_at'),an=latest(ctx.analyses,'created_at'),so=latest(ctx.supplierOffers,'created_at'),oo=latest(ctx.ourOffers,'created_at');var activity=Math.max(ms(p.created_at),ms(p.last_activity_at),e&&e._when||0,t&&t._when||0,an&&an._when||0,so&&so._when||0,oo&&oo._when||0);return{row:p,activity:activity,next:nextForProject(ctx,actions),context:ctx};}).sort(function(a,b){return b.activity-a.activity;});}
async function loadData(){
 ...
 var rows=await Promise.all([
  q('projects?select=id,created_at,name,client,ref,location,deadline,notes,status,pipeline_stage,deal_type,business_ref,business_type,last_activity_at,last_email_at&limit=3000'),
...
async function reconcile(data,contexts){
 var writes=[];Object.keys(contexts).forEach(function(pid){var ctx=contexts[pid],p=ctx.project||{};arr(ctx.tasks).forEach(function(t){if(taskIsSuperseded(ctx,t))writes.push(db('tasks?id=eq.'+enc(t.id),'PATCH',{status:'mbyllur',done_at:new Date().toISOString()}));});var sq=supplierQuoteReady(ctx),o=latestOurOffer(ctx),sent=o?sentTimeForOffer(ctx,o):0;if(sq&&!sent&&norm(p.pipeline_stage)!=='pricing')writes.push(db('projects?id=eq.'+enc(pid),'PATCH',{pipeline_stage:'pricing',updated_at:new Date().toISOString()}));if(sent&&norm(p.pipeline_stage)!=='client_offer')writes.push(db('projects?id=eq.'+enc(pid),'PATCH',{pipeline_stage:'client_offer',updated_at:new Date().toISOString()}));if(sent){var se=arr(ctx.emails).filter(function(e){return isOutgoing(e)&&ms(e.sent_at)===sent;})[0];var dl=extractExecutionDeadline((se&&se.snippet)||'');if(dl&&String(p.deadline||'')!==dl)writes.push(db('projects?id=eq.'+enc(pid),'PATCH',{deadline:dl,updated_at:new Date().toISOString()}));}});if(writes.length)try{await Promise.all(writes);}catch(e){console.warn('PPPP Home reconcile partially failed',e);}
}
...
function projectHtml(x){var p=x.row||{};return'<div class="pst-ws-projectcard pst-canonical-project" data-project-id="'+esc(p.id)+'"><div class="pst-ws-projectcard-top"><div><div class="pst-ws-projectcard-name">'+esc(p.name||'Pa emër')+'</div><div class="pst-ws-projectcard-client">'+esc((p.client||'Pa klient')+(p.ref?' · '+p.ref:''))+'</div></div><span class="pst-ws-status">'+esc(statusLabel(p.status))+'</span></div><div class="pst-ws-projectcard-next"><b>Hapi i radhës:</b> '+esc(x.next||stageName(p.pipeline_stage))+'</div></div>';}
...
function emptyProjects(){return'<div class="pst-ws-empty"><b>Nuk u gjetën projekte aktive.</b><span>Kontrollo regjistrin e projekteve.</span></div>';}
...
function briefMissing(ctx){var a=[],sq=supplierQuoteReady(ctx),w=waitingAfterOffer(ctx),p=ctx.project||{};if(sq)a.push('Oferta PRISTEEL për klientin');if(w)a.push('Përgjigjja ose konfirmimi i klientit');if(norm(p.pipeline_stage)==='technical_review')a.push('Mbyllja e sqarimeve teknike para ofertës');if(!a.length){var t=latestOpenTask(ctx);if(t&&!taskIsSuperseded(ctx,t))a.push(specificTaskTitle(ctx,t));}return a;}
...
function finalHomeText(page){var titles=page.querySelectorAll('.pst-ws-card-title'),subs=page.querySelectorAll('.pst-ws-card-sub');if(titles[0])titles[0].textContent='Për mua tani';if(subs[0])subs[0].textContent='Maksimumi pesë veprime konkrete që kërkojnë vendim ose veprim.';var cards=Array.prototype.slice.call(page.querySelectorAll('#pst-ws-home-actions > .pst-ws-action'));cards.forEach(function(x,i){x.classList.toggle('pst-hcc-hidden',i>=5);});var more=page.querySelector('#pst-ws-home-actions > .pst-hcc-more');if(more&&cards.length<=5)more.remove();}
function decorate(){try{if(window.PSTHomeCommandCenterV2&&typeof window.PSTHomeCommandCenterV2.decorate==='function')window.PSTHomeCommandCenterV2.decorate();}catch(e){}try{if(window.PSTDashboardTaskCardsV1&&typeof window.PSTDashboardTaskCardsV1.decorate==='function')window.PSTDashboardTaskCardsV1.decorate();}catch(e){}try{if(window.PSTTaskSourceActionsV1&&typeof window.PSTTaskSourceActionsV1.decorate==='function')window.PSTTaskSourceActionsV1.decorate();}catch(e){}try{if(window.PSTRedesignFinalizerV1&&typeof window.PSTRedesignFinalizerV1.apply==='function')window.PSTRedesignFinalizerV1.apply();}catch(e){}var page=document.getElementById('page-workspace-home');if(page){finalHomeText(page);[80,260,620,1120].forEach(function(ms){setTimeout(function(){finalHomeText(page);},ms);});}}
function installCss(){
...
window.PSTHomeCanonicalV1={render:render,refresh:function(){return render(true);},activateHome:activateHome,openBrief:openProjectBriefByProject,_test:{inactiveProject:inactiveProject,relDays:relDays,actionStateKey:actionStateKey,taskScore:taskScore,deriveProjects:deriveProjects,buildActions:buildActions,recommendation:recommendation,supplierQuoteReady:supplierQuoteReady,clientReplyAfterOffer:clientReplyAfterOffer,taskIsSuperseded:taskIsSuperseded},snapshot:function(){return{actions:cache.actions.slice(),waiting:cache.waiting.slice(),projects:cache.projects.slice()};}};
installCss();installBrief();setTimeout(function(){render(true);},0);document.addEventListener('pst:modules-ready',function(){render(true);},{once:true});window.addEventListener('pageshow',function(){if(document.getElementById('page-workspace-home'))render(true);},{once:true});document.addEventListener('visibilitychange',function(){if(document.visibilityState==='visible'&&document.getElementById('page-workspace-home')&&document.getElementById('page-workspace-home').classList.contains('active'))render(true);});setInterval(function(){var p=document.getElementById('page-workspace-home');if(p&&p.classList.contains('active')&&p.style.display!=='none')render(false);},AUTO_REFRESH_MS);
})();
