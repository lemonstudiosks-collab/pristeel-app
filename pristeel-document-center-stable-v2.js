...
function safeItems(r){var v=r&&((r.items!=null?r.items:null)||(r.invoice_items!=null?r.invoice_items:null)||(r.line_items!=null?r.line_items:null));for(var i=0;i<3&&typeof v==='string';i++){try{v=JSON.parse(v);}catch(e){break;}}if(Array.isArray(v))return v;if(v&&typeof v==='object'){var x=v.items||v.positions||v.rows||v.line_items;if(Array.isArray(x))return x;}return[];}
function grossInvoice(r){return num(r&&((r.gross_amount!=null?r.gross_amount:null)||(r.total_price!=null?r.total_price:null)||(r.total_eur!=null?r.total_eur:null)));}
function toast(text){var old=document.getElementById('pst-doc-toast');if(old)old.remove();var e=document.createElement('div');e.id='pst-doc-toast';e.textContent=String(text||'');document.body.appendChild(e);setTimeout(function(){if(e.parentNode)e.remove();},4200);}
function syncCompat(){
  var C=window.PST_DOC_CENTER||{};
  C.labels={offer:{name:'Ofertë'},invoice:{name:'Faturë'},credit_note:{name:'Notë Kreditore'},debit_note:{name:'Notë Debitore'}};
  C.icons=C.icons||{close:'✕'};
  C.arr=arr;C.esc=esc;C.num=num;C.money=money;C.dateText=dateText;C.safeItems=safeItems;C.grossInvoice=grossInvoice;C.toast=toast;
  C.invoices=D.invoices;C.adjustments=D.adjustments;C.adjustmentsReady=true;
  window.PST_DOC_CENTER=C;
}
syncCompat();
...
function addCss(){if(document.getElementById('pst-document-center-css'))return;var l=document.createElement('link');l.id='pst-document-center-css';l.rel='stylesheet';l.href='pristeel-document-center.css?v=20260808-2';document.head.appendChild(l);}
...
function createSelected(){var t=D.selectedType;if(window.PSTCommercialNavigationFixV1&&typeof window.PSTCommercialNavigationFixV1.createDocument==='function'){window.PSTCommercialNavigationFixV1.createDocument(t);return;}if(t==='offer'){if(typeof window.pstWsLegacy==='function')window.pstWsLegacy('oferta');else if(typeof window.showPage==='function')window.showPage('oferta');return;}if(t==='invoice'){if(typeof window.pstWsLegacy==='function')window.pstWsLegacy('invoices');else if(typeof window.showPage==='function')window.showPage('invoices');return;}if(typeof window.pstOpenAdjustment==='function')window.pstOpenAdjustment(t,'');}
...
function load(){
  ...
  D.loading=Promise.all([
    ...
    D.quotes=out[0];D.invoices=out[1];D.adjustments=out[2];syncCompat();
    ...
    render();
    return D.all;
  }).catch(function(){return D.all;}).finally(function(){D.loading=null;});
  return D.loading;
}
...
document.addEventListener('pst:modules-ready',function(){ensurePage();decorateNav();syncCompat();},{once:true});
