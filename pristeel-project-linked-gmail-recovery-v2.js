...
function markRecommended(files){var groups={};files.forEach(function(f){var k=versionFamily(f);(groups[k]=groups[k]||[]).push(f);});Object.keys(groups).forEach(function(k){groups[k].sort(function(a,b){return rank(b)-rank(a);}).forEach(function(f,i){f.recommended=i===0;});});}
...
function render(){
  ...
  var fileRows=state.files.map(function(f){var checked=f.recommended?' checked':'',copies=Number(f.duplicateCount||1)>1?' · '+Number(f.duplicateCount||1)+' kopje në emaila':'';return'<label class="plgr-file"><input type="checkbox" class="plgr-check" value="'+esc(f.key)+'"'+checked+'><div class="plgr-filemain"><b>'+esc(f.filename)+'</b><span>'+esc(fileKind(f))+' · '+esc(fmtDate(f.sentAt))+' · '+esc(fmtBytes(f.size))+esc(copies)+'</span></div><a target="_blank" href="'+esc(f.gmailUrl)+'">Emaili</a></label>';}).join('');
...
window.PSTLinkedGmailRecoveryV2={open:open,_test:{isLikelySignature:isLikelySignature,dedupeKey:dedupeKey,collapseRepeatedFiles:collapseRepeatedFiles,versionFamily:versionFamily,markRecommended:markRecommended}};
