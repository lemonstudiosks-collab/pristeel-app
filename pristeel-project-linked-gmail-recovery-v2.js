...
var state={projectId:'',project:null,rows:[],messages:[],files:[],token:'',busy:false};
...
function markRecommended(files){var groups={};files.forEach(function(f){var k=baseName(f.filename)||String(f.filename||'').toLowerCase();(groups[k]=groups[k]||[]).push(f);});Object.keys(groups).forEach(function(k){groups[k].sort(function(a,b){return rank(b)-rank(a);}).forEach(function(f,i){f.recommended=i===0&&!isLikelySignature(f);});});}
...
async function loadData(){
  ...
  try{
    ...
    state.files=uniq(files,function(f){return f.key;});markRecommended(state.files);render();
    setStatus(state.files.length?'U gjetën '+state.files.length+' skedarë vetëm në emailat e këtij projekti.':'Emailat janë lidhur, por nuk kanë attachment-e reale.','ok',100);
  }catch(e){setStatus('Gabim: '+String(e&&e.message||e),'err',0);}
  finally{setBusy(false);}
}
function render(){
  var host=document.getElementById('plgr-results');if(!host)return;
  var real=state.files.filter(function(f){return!isLikelySignature(f);}),selected=real.filter(function(f){return f.recommended;});
  ...
  var fileRows=state.files.map(function(f){var sig=isLikelySignature(f),checked=f.recommended?' checked':'',disabled=sig?' disabled':'';return'<label class="plgr-file'+(sig?' muted':'')+'"><input type="checkbox" class="plgr-check" value="'+esc(f.key)+'"'+checked+disabled+'><div class="plgr-filemain"><b>'+esc(f.filename)+'</b><span>'+esc(fileKind(f))+' · '+esc(fmtDate(f.sentAt))+' · '+esc(fmtBytes(f.size))+(sig?' · inline/nënshkrim':'')+'</span></div><a target="_blank" href="'+esc(f.gmailUrl)+'">Emaili</a></label>';}).join('');
  host.innerHTML='<div class="plgr-kpis"><div><b>'+state.rows.length+'</b><span>Emaila të lidhur</span></div><div><b>'+real.length+'</b><span>Skedarë realë</span></div><div><b>'+selected.length+'</b><span>Të zgjedhur</span></div></div><h4>Skedarët e emailave të këtij projekti</h4><div class="plgr-list">'+(fileRows||'<div class="plgr-empty">Nuk u gjet asnjë attachment real.</div>')+'</div><details><summary>Emailat e lidhur</summary><div class="plgr-mails">'+(messageRows||'<div class="plgr-empty">Nuk ka emaila.</div>')+'</div></details>';
}
...
function open(projectId){
  state={projectId:String(projectId||''),project:null,rows:[],messages:[],files:[],token:'',busy:false};
