...
`pristeel-mobile-home-v1.js` remains loaded on <=900px devices as the bounded utility/public-source provider for `Pyet PPPP`, live weather, ECB FX and steel-market source actions. `pristeel-mobile-control-tower-v1.js` is the later mobile Home presentation owner: it consumes the existing `PSTHomeCanonicalV1.snapshot()` so the phone Home answers what needs attention now, what is waiting externally and which projects are actively in work, without creating a second business/data engine. The fixed bottom navigation remains the primary route to Mundësitë, Projektet, Partnerët, Financat and Sistemi. A compact utility dock keeps Weather, Convert and Market visible and delegates them to the existing provider. The Control Tower itself adds no Supabase reads/writes, network fetches or polling; bounded public reads remain owned by `pristeel-mobile-home-v1.js`.

On a trusted <=900px device, `pristeel-mobile-pin-unlock-v1.js` is the normal re-entry gate after initial account authentication and PIN setup. Access-session expiry must not send the user back to email/password: PIN verification restores the remembered refresh-token session and delegates token renewal to the existing auth refresh path. Explicit logout remains the deliberate action that clears the trusted-device PIN/session state.
