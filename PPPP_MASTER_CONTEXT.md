...
PPPP uses the same application, routes, backend and business owners across desktop, tablet and phone. The earlier responsive/mobile modules remain loaded for compatibility, safe-area handling and utility actions, but `pristeel-mobile-app-v2.js` is the final <=900px presentation owner.

Mobile App v2 presents a dedicated phone/tablet shell instead of exposing the desktop/mobile hybrid directly. Its primary navigation is **Home, Projects, Discover, Inbox** with a raised central **+** action. Secondary daily zones — Partnerët, Financat and Sistemi — plus Material Trade and Përfaqësime remain reachable from the profile/more sheet and continue to open their existing canonical owners.

The mobile shell does not create a second business engine. Home consumes `PSTHomeCanonicalV1.snapshot()`; Projects reuses current project caches/owners; Discover delegates to Tender Priority Actions, Material Trade and Representations; Inbox delegates to Gmail Live Inbox. The four primary pages — Home, Projects, Discover and Inbox — live on one full-screen horizontal pager and can be changed either with the bottom navigation or with a native-feeling left/right swipe. Nested card carousels are deliberately avoided so page swipe has one clear gesture owner. Any business-state change remains tied to an explicit user action, and protected commercial actions keep their human approval gates.

`pristeel-mobile-home-v1.js` remains loaded only as the bounded utility/public-source provider for `Pyet PPPP`, live weather, ECB FX and steel-market actions. The final mobile shell mounts outside the legacy app container and fully covers the legacy dashboard while active, so the old weather/market Home cannot appear above or below the new UI. Compact Weather / Convert / Market shortcuts remain available without becoming primary Home content. It adds no direct Supabase reads/writes, no polling and no independent network fetches.

On a trusted <=900px device, `pristeel-mobile-pin-unlock-v1.js` remains the normal re-entry gate after initial account authentication and PIN setup. Access-session expiry must not send the user back to email/password: PIN verification restores the remembered refresh-token session and delegates token renewal to the existing auth refresh path. Explicit logout remains the deliberate action that clears the trusted-device PIN/session state. Installable-app metadata remains supplied through `pristeel.webmanifest`; service-worker caching stays deliberately deferred to avoid stale-runtime risk.
