...
The mobile shell does not create a second business engine. Home consumes `PSTHomeCanonicalV1.snapshot()`; Projects reuses current project caches/owners; Discover delegates to Tender Priority Actions, Material Trade and Representations; Inbox delegates to Gmail Live Inbox. Home priority cards use native horizontal swipe and Discover opportunities use a touch swipe deck. Any business-state change remains tied to an explicit user action, and protected commercial actions keep their human approval gates.

`pristeel-mobile-home-v1.js` remains loaded only as the bounded utility/public-source provider for `Pyet PPPP`, live weather, ECB FX and steel-market actions. Mobile App v2 renders those as compact fixed utility icons. It adds no direct Supabase reads/writes, no polling and no independent network fetches. The old six-tab mobile bar, old information dashboard and interim Control Tower presentation yield while Mobile App v2 is active.
