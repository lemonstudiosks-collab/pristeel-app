...
var VERSION='20260926-mobile-app2';
...
function showShell(tab){
  state.detail=false;state.tab=tab||state.tab||'home';document.body&&document.body.classList.add('pst-mobile-v2-active');
  render();
  try{scrollTo({top:0,behavior:'auto'});}catch(e){}
}
...
function discoverTenders(){
  ...
  return '<div class="pma-deck-wrap"><div class="pma-deck-counter"><span>'+E((idx+1)+' nga '+rows.length)+'</span><small>Rrëshqit kartën majtas / djathtas</small></div>'+
    '<div class="pma-deck" data-pma-deck data-id="'+E(r.id)+'">'+
      ...
      '</div>'+
      '<div class="pma-swipe-hint"><span>← e mëparshmja</span><span>tjetra →</span></div>'+
      '</article>'+
    '</div></div>';
}
...
function rootMarkup(){
  var body=state.tab==='projects'?projectsView():state.tab==='discover'?discoverView():state.tab==='inbox'?inboxView():homeView();
  return '<div class="pma-shell">'+body+'</div>';
}
function plusSheet(){
...
function render(){
  ...
  var h=host(),r=document.getElementById(ROOT);
  if(!r){r=document.createElement('main');r.id=ROOT;r.setAttribute('aria-label','PriSteel Mobile');h.insertBefore(r,h.firstChild||null);}
  if(!state.detail){
  ...
  ensureChrome();syncNav();bindDeck();return true;
}
...
function bindDeck(){
  var deck=document.querySelector('#'+ROOT+' [data-pma-deck]');if(!deck||deck.dataset.bound==='1')return;deck.dataset.bound='1';
  var x0=0,y0=0,dx=0,drag=false;
  deck.addEventListener('touchstart',function(e){var t=e.touches&&e.touches[0];if(!t)return;x0=t.clientX;y0=t.clientY;dx=0;drag=true;deck.style.transition='none';},{passive:true});
  deck.addEventListener('touchmove',function(e){if(!drag)return;var t=e.touches&&e.touches[0];if(!t)return;dx=t.clientX-x0;var dy=t.clientY-y0;if(Math.abs(dx)>Math.abs(dy)){deck.style.transform='translateX('+dx+'px) rotate('+(dx/28)+'deg)';}},{passive:true});
  deck.addEventListener('touchend',function(){if(!drag)return;drag=false;deck.style.transition='transform .22s ease';if(Math.abs(dx)>70){state.opportunityIndex+=dx<0?1:-1;if(state.opportunityIndex<0)state.opportunityIndex=state.opportunities.length-1;if(state.opportunityIndex>=state.opportunities.length)state.opportunityIndex=0;deck.style.transform='translateX('+(dx<0?-420:420)+'px) rotate('+(dx<0?-12:12)+'deg)';setTimeout(render,180);}else deck.style.transform='';});
}
function click(e){
  var t=e.target&&e.target.closest?e.target:null;if(!t)return;
  var b=t.closest('[data-pma-tab]');if(b){e.preventDefault();state.plusOpen=false;state.moreOpen=false;showShell(b.getAttribute('data-pma-tab'));if(state.tab==='discover'&&!state.opportunities.length)loadOpportunities(false);if(state.tab==='inbox')loadInbox(false);return;}
