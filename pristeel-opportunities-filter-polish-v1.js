...
var VERSION='20260921-opportunity-desk1';
...
var WINNERS=[
 {id:'gc_epc',label:'GC / EPC',hint:'klient potencial'},
 {id:'other',label:'Për verifikim',hint:'rol i paqartë / konsorcium'},
 {id:'producer',label:'Prodhues çeliku',hint:'kapacitet / konkurrencë'}
];
...
.pst-opp-filter-panel{display:grid;gap:13px;padding:16px 17px;border:1px solid #dfe7e9;border-radius:16px;background:#fff}.pst-opp-filter-panel[hidden]{display:none!important}.pst-opp-filter-row{display:grid;grid-template-columns:135px minmax(0,1fr);gap:12px;align-items:start}.pst-opp-filter-label{padding-top:7px;font-size:9px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:#7b8d94}.pst-opp-filter-options{display:flex;flex-wrap:wrap;gap:7px}.pst-opp-chip{min-height:32px;padding:0 11px;border:1px solid #dce6e9;border-radius:999px;background:#fbfcfd;color:#536b75;font-size:10px;font-weight:780;cursor:pointer}.pst-opp-chip:hover{border-color:#b5cfd8;background:#f3f8f9}.pst-opp-chip.on{border-color:#8fbcc9;background:#eaf5f8;color:#2f748c}.pst-opp-chip .count{margin-left:6px;opacity:.65}
...
@media(max-width:980px){.pst-opp-route-grid{grid-template-columns:1fr}.pst-opp-route{min-height:92px}.pst-opp-mini-stats{grid-template-columns:repeat(2,1fr)}.pst-opp-mini-stat:nth-child(2){border-right:0}.pst-opp-filter-row{grid-template-columns:1fr}.pst-opp-filter-label{padding-top:0}.pst-opp-desk-title p{display:none}}
@media(max-width:650px){body:has(#page-kek-tenders.active) .content{padding:12px 13px 34px!important}#pst-opportunities-focus>header{align-items:flex-start!important}.pst-opp-desk-head-left{gap:9px}.pst-opp-desk-title h2{font-size:22px!important}.pst-opp-density{display:none}.pst-opp-mini-stats{grid-template-columns:1fr}.pst-opp-mini-stat{border-right:0;border-bottom:1px solid #e8edef}.pst-opp-mini-stat:last-child{border-bottom:0}#pst-pcw-opportunity-tools{align-items:stretch!important}.pst-opp-filter-toggle{padding:0 11px}.pst-pcw-tender{grid-template-columns:1fr!important}.pst-pcw-tender-open{justify-content:flex-end}}
...
function chip(attr,id,label,count,on){return'<button type="button" class="pst-opp-chip '+(on?'on':'')+'" '+attr+'="'+E(id)+'">'+E(label)+(count==null?'':'<span class="count">'+Number(count||0)+'</span>')+'</button>';}
...
function sourceRow(c){var active=SOURCES.filter(function(k){return Number(c.src[k]||0)>0;});return chip('data-pst-opp-source','all','Të gjitha burimet',c.total,state&&state.source==='all')+active.map(function(k){return chip('data-pst-opp-source',k,LABEL[k]||k,c.src[k],state&&state.source===k);}).join('');}
function fieldRow(c){return chip('data-pst-opp-field','all','Të gjitha',null,state&&state.field==='all')+FIELDS.map(function(f){return chip('data-pst-opp-field',f.id,f.label,c.field[f.id],state&&state.field===f.id);}).join('');}
function winnerRow(c){return chip('data-pst-opp-winner','all','Të gjithë fituesit',c.award,state&&state.winner_group==='all')+WINNERS.map(function(w){return chip('data-pst-opp-winner',w.id,w.label,c.winner[w.id],state&&state.winner_group===w.id);}).join('');}
