/* PRISTEEL Gmail Live Inbox v2
 * Stable, bounded Gmail project-intake entry.
 * Read-only until the user explicitly starts intake.
 */
(function(){
'use strict';
if(window.__pstGmailLiveInboxV2)return;
window.__pstGmailLiveInboxV2=true;
var state={busy:false,rows:[],canonicalRows:[],canonicalLoaded:false,canonicalLoading:false,error:'',query:'',seq:0};
var LIST_WAIT=4500,MSG_WAIT=3200;
function arr(v){return Array.isArray(v)?v:[];}
function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');}
function enc(v){return encodeURIComponent(String(v==null?'':v));}
function short(v,n){v=String(v||'').replace(/\s+/g,' ').trim();return v.length>n?v.slice(0,n-1)+'…':v;}
function date(v){var d=v?new Date(v):null;return d&&!isNaN(d.getTime())?d.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit',year:'numeric'}):'';}
function token(){var G=window.PSTGoogleWorkspaceAuth;return G&&G.currentToken?G.currentToken([G.gmailScope]):'';}
function gmail(path,t){return window.PSTEmail&&window.PSTEmail.gmail?window.PSTEmail.gmail(path,t):Promise.reject(new Error('Gmail core nuk është gati.'));}
function message(id,t){return window.PSTEmail&&window.PSTEmail.message?window.PSTEmail.message(id,t):Promise.reject(new Error('Gmail core nuk është gati.'));}
function bounded(promise,ms){return new Promise(function(resolve,reject){var done=false,t=setTimeout(function(){if(done)return;done=true;reject(new Error('Kërkesa mori shumë kohë. Provo përsëri.'));},ms);Promise.resolve(promise).then(function(v){if(done)return;done=true;clearTimeout(t);resolve(v);}).catch(function(e){if(done)return;done=true;clearTimeout(t);reject(e);});});}
function setStatus(text,bad){var e=document.getElementById('pst-gli-status');if(!e)return;e.textContent=text||'';e.style.color=bad?'#8A5A52':'#6E7C83';}
function navLabel(){var b=document.querySelector('.pst-ws-navbtn[data-key="inbox"] span');if(b)b.textContent='Gmail / Inbox';}
function css(){
  if(document.getElementById('pst-gmail-live-inbox-readable-css'))return;
  var s=document.createElement('style');s.id='pst-gmail-live-inbox-readable-css';s.textContent=`
#page-workspace-inbox .pst-ws-eyebrow{font-size:10.5px!important;margin-bottom:6px!important}
#page-workspace-inbox .pst-ws-title{font-size:28px!important;line-height:1.16!important}
#page-workspace-inbox .pst-ws-sub{font-size:12.5px!important;line-height:1.45!important;margin-top:7px!important}
#page-workspace-inbox .pst-ws-btn{height:40px!important;padding:0 14px!important;font-size:12.5px!important}
#page-workspace-inbox .pst-ws-btn svg{width:16px!important;height:16px!important}
#page-workspace-inbox .pst-ws-card-hd{padding:16px 18px!important}
#page-workspace-inbox .pst-ws-card-title{font-size:14px!important;line-height:1.35!important}
#page-workspace-inbox .pst-ws-card-sub{font-size:11.5px!important;line-height:1.45!important;margin-top:3px!important}
#page-workspace-inbox .pst-ws-card-body{padding:11px 12px 14px!important}
#page-workspace-inbox .pst-ws-input{font-size:13px!important;min-height:40px!important;padding-left:13px!important;padding-right:13px!important}
#page-workspace-inbox .pst-ws-action{padding:13px 9px!important;gap:11px!important}
#page-workspace-inbox .pst-ws-action-title{font-size:13px!important;line-height:1.4!important}
#page-workspace-inbox .pst-ws-action-meta{font-size:11.5px!important;line-height:1.45!important;margin-top:3px!important}
#page-workspace-inbox .pst-ws-rowaction{font-size:12px!important;min-height:34px!important;padding:0 10px!important}
#page-workspace-inbox .pst-ws-empty{font-size:12px!important;line-height:1.5!important;padding:30px 16px!important}
#page-workspace-inbox #pst-gli-status{font-size:11.5px!important;line-height:1.45!important;margin-bottom:10px!important}
#page-workspace-inbox #pst-gmail-live-card{margin-bottom:16px!important}
#page-workspace-inbox .pst-gli-dismiss{color:#7B858A!important;background:#fff!important;border-color:#DCE5E8!important}
#page-workspace-inbox .pst-gli-dismiss:hover{color:#9A3F35!important;background:#FBF2F1!important;border-color:#E8C9C5!important}
#pst-gli-list .pst-ws-action:has(.pst-glt-tag.bounce) .pst-gli-dismiss{display:none!important}
@media(max-width:800px){#page-workspace-inbox .pst-ws-title{font-size:25px!important}#page-workspace-inbox .pst-ws-btn{height:38px!important;padding:0 11px!important;font-size:12px!important}}
`;
  document.head.appendChild(s);
}
function shell(){navLabel();var page=document.getElementById('page-workspace-inbox');if(!page||page.style.display==='none'||!page.classList.contains('active'))return null;var host=page.querySelector('.pst-ws-page');if(!host)return null;var old=document.getElementById('pst-gmail-live-card');if(old)return old;var head=host.querySelector('.pst-ws-head'),card=document.createElement('section');card.id='pst-gmail-live-card';card.className='pst-ws-card';card.style.marginBottom='14px';card.innerHTML='<div class="pst-ws-card-hd"><div><div class="pst-ws-card-title">Gmail live</div><div class="pst-ws-card-sub">Zgjidh një email real dhe nis projektin prej thread-it të tij</div></div><div style="display:flex;gap:7px;align-items:center"><button class="pst-ws-btn" id="pst-gli-auth">Lidhu me Gmail</button><button class="pst-ws-btn" id="pst-gli-refresh">Rifresko</button></div></div><div class="pst-ws-card-body"><div class="pst-ws-toolbar" style="padding:0 0 10px"><input class="pst-ws-input" id="pst-gli-query" placeholder="Kërko në Gmail: kompani, projekt, subjekt…"><button class="pst-ws-btn primary" id="pst-gli-search">Kërko</button></div><div id="pst-gli-status" style="font-size:10px;color:#6E7C83;margin-bottom:8px"></div><div id="pst-gli-list"><div class="pst-ws-empty">Gmail është gati.</div></div></div>';if(head&&head.nextSibling)host.insertBefore(card,head.nextSibling);else host.insertBefore(card,host.firstChild);card.querySelector('#pst-gli-auth').onclick=authorize;card.querySelector('#pst-gli-refresh').onclick=function(){load(true);};card.querySelector('#pst-gli-search').onclick=function(){load(true);};card.querySelector('#pst-gli-query').onkeydown=function(e){if(e.key==='Enter')load(true);};updateAuth();return card;}
function updateAuth(){var b=document.getElementById('pst-gli-auth'),t=token();if(!b)return;b.textContent=t?'Gmail i lidhur':'Lidhu me Gmail';b.disabled=!!t;}
function render(seq){if(seq!==state.seq)return;var h=document.getElementById('pst-gli-list');if(!h)return;if(!state.rows.length){h.innerHTML='<div class="pst-ws-empty">Nuk u gjet asnjë email.</div>';return;}h.innerHTML=state.rows.map(function(r){return'<div class="pst-ws-action" style="align-items:center"><i class="pst-ws-action-dot" style="--c:#3F7F98;--bg:#EAF4F7"></i><div class="pst-ws-action-main"><div class="pst-ws-action-title">'+esc(r.subject||'(pa subjekt)')+'</div><div class="pst-ws-action-meta">'+esc(r.from_name||r.from_email||'')+' · '+date(r.sent_at)+(r.has_attachments?' · Ka bashkëngjitje':'')+'</div><div class="pst-ws-action-meta" style="margin-top:2px">'+esc(short(r.snippet,145))+'</div></div><div style="display:flex;gap:5px;flex-shrink:0"><button class="pst-ws-rowaction" data-gmail-url="'+esc(r.gmail_url||'')+'">Hap Gmail</button><button class="pst-ws-rowaction pst-gli-intake" style="background:#3F7F98;color:#fff;border-color:#3F7F98" data-mid="'+esc(r.gmail_message_id)+'" data-tid="'+esc(r.gmail_thread_id)+'">Krijo / Lidhe projektin</button><button class="pst-ws-rowaction pst-gli-dismiss" data-dismiss-mid="'+esc(r.gmail_message_id)+'" title="Largo vetëm nga PPPP">Hiqe</button></div></div>';}).join('');h.querySelectorAll('[data-gmail-url]').forEach(function(b){b.onclick=function(){var u=b.getAttribute('data-gmail-url');if(u)window.open(u,'PRISTEEL_GMAIL');};});h.querySelectorAll('.pst-gli-intake').forEach(function(b){b.onclick=function(){intake(b.getAttribute('data-mid'),b.getAttribute('data-tid'));};});h.querySelectorAll('.pst-gli-dismiss').forEach(function(b){b.onclick=function(){dismissManual(b.getAttribute('data-dismiss-mid'),b);};});}
async function dismissManual(mid,btn){mid=String(mid||'').trim();if(!mid)return;var r=state.rows.filter(function(x){return String(x.gmail_message_id)===mid;})[0]||{};if(!window.confirm('Ta heqim këtë email vetëm nga PPPP? Emaili në Gmail nuk fshihet.'))return;var old=btn&&btn.textContent;if(btn){btn.disabled=true;btn.textContent='Duke hequr…';}try{if(typeof window.supaFetch!=='function')throw new Error('Databaza nuk është gati.');var found=await window.supaFetch('dismissed_items?select=id&item_type=eq.gmail_live&item_ref=eq.'+enc(mid)+'&limit=1');if(!Array.isArray(found)||!found.length)await window.supaFetch('dismissed_items','POST',{item_type:'gmail_live',item_ref:mid,project_id:null,label:r.subject||null,reason:'manual_not_relevant',dismissed_by:null,dismissed_at:new Date().toISOString()});state.rows=state.rows.filter(function(x){return String(x.gmail_message_id)!==mid;});var row=btn&&btn.closest('.pst-ws-action');if(row)row.remove();setStatus('Emaili u hoq nga PPPP. Gmail mbeti i paprekur.');}catch(e){if(btn){btn.disabled=false;btn.textContent=old||'Hiqe';}setStatus((e&&e.message)||String(e),true);}}
async function loadCanonical(force){
 if(state.canonicalLoading)return state.canonicalRows;
 if(state.canonicalLoaded&&!force)return state.canonicalRows;
 if(typeof window.supaFetch!=='function'){state.error='Databaza nuk është gati.';return state.canonicalRows;}
 state.canonicalLoading=true;state.error='';
 try{
  var path='project_emails?select=id,project_id,suggested_project_id,gmail_message_id,gmail_thread_id,from_email,from_name,subject,snippet,sent_at,direction,has_attachments,gmail_url,needs_review,review_reason&direction=eq.incoming&order=sent_at.desc.nullslast&limit=80';
  var rows=arr(await window.supaFetch(path)),seen={};
  state.canonicalRows=rows.filter(function(r){var k=String(r.gmail_thread_id||r.gmail_message_id||r.id||'');if(!k||seen[k])return false;seen[k]=1;return true;}).slice(0,30);
  state.canonicalLoaded=true;
 }catch(e){state.error=(e&&e.message)||String(e);}
 state.canonicalLoading=false;return state.canonicalRows;
}
async function load(interactive){shell();if(state.busy)return;var t=token();if(!t){updateAuth();setStatus('Kliko “Lidhu me Gmail” për të lexuar inbox-in real.');var empty=document.getElementById('pst-gli-list');if(empty)empty.innerHTML='<div class="pst-ws-empty">Gmail nuk është autorizuar në këtë sesion.</div>';return;}state.busy=true;var seq=++state.seq,h=document.getElementById('pst-gli-list');setStatus('Duke lexuar Gmail…');if(h)h.innerHTML='<div class="pst-ws-empty">Duke ngarkuar emailat…</div>';try{var q=String((document.getElementById('pst-gli-query')||{}).value||'').trim();state.query=q;var path='/messages?labelIds=INBOX&maxResults=24'+(q?'&q='+enc(q):''),list=await bounded(gmail(path,t),LIST_WAIT),ids=arr(list&&list.messages).map(function(x){return x.id;}).slice(0,24),rows=[];for(var i=0;i<ids.length;i+=8){var batch=ids.slice(i,i+8),got=await Promise.all(batch.map(function(id){return bounded(message(id,t),MSG_WAIT).catch(function(){return null;});}));if(seq!==state.seq)return;rows=rows.concat(got.filter(Boolean));}var seen={};state.rows=rows.filter(function(r){var k=String(r.gmail_thread_id||r.gmail_message_id||'');if(!k||seen[k])return false;seen[k]=1;return true;}).slice(0,20);state.error='';render(seq);setStatus(state.rows.length+' thread-e nga Gmail'+(q?' për kërkimin “'+q+'”':'')+'.');}catch(e){if(seq===state.seq){state.error=(e&&e.message)||String(e);setStatus(state.error,true);if(h)h.innerHTML='<div class="pst-ws-empty">Gmail nuk u ngarkua. Provo Rifresko.</div>';}}finally{if(seq===state.seq)state.busy=false;updateAuth();}}
async function authorize(){var G=window.PSTGoogleWorkspaceAuth;if(!G||!G.getGmailToken){setStatus('Moduli i autorizimit Gmail nuk është gati.',true);return;}try{setStatus('Po hap autorizimin Google…');await bounded(G.getGmailToken({interactive:true}),12000);updateAuth();await load(true);}catch(e){setStatus((e&&e.message)||String(e),true);}}
function intake(mid,tid){var r=state.rows.filter(function(x){return String(x.gmail_message_id)===String(mid);})[0]||{};try{var u=new URL(location.href);u.searchParams.set('gmail_intake','1');u.searchParams.set('gmail_message_id',mid);u.searchParams.set('gmail_thread_id',tid||r.gmail_thread_id||'');u.searchParams.set('subject',r.subject||'');u.searchParams.set('from',r.from_email||'');var target=u.href;if(window.PSTGmailIntakeV2&&window.PSTGmailIntakeV2.open)return window.PSTGmailIntakeV2.open(target);if(window.PSTGmailHandoffV4&&window.PSTGmailHandoffV4.openTarget)return window.PSTGmailHandoffV4.openTarget(target);location.href=target;}catch(e){setStatus((e&&e.message)||String(e),true);}}
window.pstGmailLiveAuthorize=authorize;window.pstGmailLiveLoad=load;window.pstGmailLiveIntake=intake;window.pstGmailLiveDismiss=dismissManual;
function decorate(){var c=shell();if(c){if(token())load(false);else setStatus('Kliko “Lidhu me Gmail” për të lexuar inbox-in real.');}}
var original=window.pstWorkspaceGo;if(typeof original==='function'&&!original.__pstGmailLiveV2Wrapped){var wrapped=function(key){var out=original.apply(this,arguments);if(key==='inbox')setTimeout(decorate,0);return out;};wrapped.__pstGmailLiveV2Wrapped=true;window.pstWorkspaceGo=wrapped;}
document.addEventListener('pst:modules-ready',function(){setTimeout(function(){navLabel();var p=document.getElementById('page-workspace-inbox');if(p&&p.classList.contains('active'))decorate();},80);},{once:true});
css();
window.PSTGmailLiveInboxV2={decorate:decorate,load:load,loadCanonical:loadCanonical,authorize:authorize,intake:intake,dismiss:dismissManual,connected:function(){return !!token()},snapshot:function(){return{liveRows:state.rows.slice(),canonicalRows:state.canonicalRows.slice(),connected:!!token(),busy:state.busy,canonicalLoading:state.canonicalLoading,error:state.error}},_state:state};navLabel();
})();