...
const VERSION='winner-company-v1';
...
const PRODUCER_RULES=[
  ...
  [/\b(stahlbau|steel construction|steel structures?|structural steel|metallbau|metal structures?)\b/gi,3,'structural-steel production'],
...
function scoreRules(raw,rules){
  ...
  for(const [pattern,weight,label] of rules){pattern.lastIndex=0;const matches=[...hay.matchAll(pattern)];if(!matches.length)continue;const capped=Math.min(2,matches.length);score+=Number(weight)*capped;evidence.push({label,weight:Number(weight)*capped,examples:unique(matches.slice(0,2).map(m=>text(m[0])))});}
  return{score,evidence};
}
export function classifyCompanyText(raw,{organizationCount=1}={}){
  if(Number(organizationCount)>1){return{company_type:'trader_consortium',confidence:'high',scores:{producer:0,gc_epc:0,trader_consortium:6},evidence:[{label:'multiple awarded organizations / consortium',weight:6,examples:[String(organizationCount)]}]};}
  ...
  const ranked=Object.entries(scores).sort((a,b)=>b[1]-a[1]);const [best,bestScore]=ranked[0],second=ranked[1]?.[1]||0;
...
function candidateLinks(html,base){
  const host=domain(base),out=[];for(const m of String(html||'').matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){
    try{const u=new URL(m[1],base);if(!/^https?:$/.test(u.protocol)||domain(u.href)!==host)continue;const label=norm(`${u.pathname} ${stripHtml(m[2])}`);if(!/(about|company|unternehmen|uber-uns|ueber-uns|services|leistungen|capabilit|production|fertigung|stahlbau|steel|construction|bau)/.test(label))continue;const clean=u.href.split('#')[0];if(!out.includes(clean))out.push(clean);if(out.length>=4)break;}catch{}
  }return out;
}
async function fetchPage(url,{fetchImpl=fetch,timeoutMs=8000}={}){const r=await fetchImpl(url,{redirect:'follow',headers:{'User-Agent':'Mozilla/5.0 (compatible; PRISTEEL-Procurement-Research/1.0; +https://prissteel.com)','Accept':'text/html,application/xhtml+xml;q=0.9,*/*;q=0.7'},signal:AbortSignal.timeout(timeoutMs)});if(!r.ok)throw new Error(`HTTP ${r.status}`);const ct=String(r.headers?.get?.('content-type')||'');if(ct&&!/html|text/i.test(ct))throw new Error('non-html');return{url:r.url||url,html:await r.text()};}
...
async function researchRow(row,{fetchImpl=fetch}={}){
  ...
  for(const seed of sites.slice(0,2)){
    try{const home=await fetchPage(seed,{fetchImpl});combined+=' '+stripHtml(home.html);sources.push(home.url);for(const u of candidateLinks(home.html,home.url).slice(0,3)){try{const p=await fetchPage(u,{fetchImpl});combined+=' '+stripHtml(p.html);sources.push(p.url);}catch{}}if(combined.length>1200)break;}catch{}
  }
  const c=classifyCompanyText(combined,{organizationCount:1});return{...c,source_urls:unique(sources).slice(0,6),text_chars:combined.length};
}
async function rest({supabaseUrl,apiKey,bearerToken=apiKey,path,method='GET',body,prefer}){const r=await fetch(`${supabaseUrl}/rest/v1/${path}`,{method,headers:{apikey:apiKey,Authorization:`Bearer ${bearerToken}`,'Content-Type':'application/json',...(prefer?{Prefer:prefer}:{})},...(body===undefined?{}:{body:JSON.stringify(body)})});const raw=await r.text();if(!r.ok)throw new Error(`${method} ${path} failed: HTTP ${r.status} ${raw.slice(0,700)}`);return raw?JSON.parse(raw):[];}
async function patchRow(access,row,result){
  const p={...(row.payload||{})},w={...winner(row)};w.company_type=result.company_type;w.company_classification={version:VERSION,company_type:result.company_type,confidence:result.confidence,scores:result.scores,evidence:result.evidence,source_urls:result.source_urls||[],classified_at:new Date().toISOString()};p.winner=w;p.workflow_track='ted_award_sales';p.business_mode='winner_outreach';p.company_verification_required=result.company_type==='unknown';p.cooperation_angle=cooperationAngle(result.company_type);
...
export async function runTedWinnerCompanyClassification({mode=process.env.SYNC_MODE||'preview',minScore=Number(process.env.TED_COMPANY_MIN_SCORE||85),maxRows=Number(process.env.TED_COMPANY_MAX_ROWS||20),fetchImpl=fetch,supabaseUrl=process.env.SUPABASE_URL||DEFAULT_SUPABASE_URL,apiKey=process.env.SUPABASE_SECRET_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY||process.env.SUPABASE_SERVICE_KEY||'',bearerToken=''}={}){
  ...
  const results=[];
  for(const row of candidates){try{const result=await researchRow(row,{fetchImpl});if(mode==='apply')await patchRow(access,row,result);results.push({id:row.id,title:row.title,company_type:result.company_type,confidence:result.confidence,scores:result.scores,source_urls:result.source_urls||[]});}catch(e){results.push({id:row.id,title:row.title,company_type:'unknown',confidence:'error',error:String(e?.message||e)});}}
  ...
  await writeSummary(summary);console.log(`TED winner company classification ${mode}: candidates=${summary.candidates}, producer=${summary.producer}, gc_epc=${summary.gc_epc}, unknown=${summary.unknown}.`);return summary;
}
