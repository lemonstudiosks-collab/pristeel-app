...
export function classify(record){
  const titleN=norm(record.title),c=norm([record.title,record.body,record.contract_type,record.document_type].filter(Boolean).join(' '));let sector='other',score=20;const reasons=[];
  ...
  const serviceTitle=/\b(specialist|consultant|consultancy|consulting|supervision|study|assessment|training|software development|portal)\b/.test(titleN),physicalTitle=/steel|metalwork|fabrication|construction works|civil works|building works|supply and installation|machinery|equipment|rehabilitation works|remedial works/.test(titleN);
  const consulting=(serviceTitle&&!physicalTitle)||(/consultancy|consulting services|construction supervision|design and supervision|training|study|assessment|software|portal solution|conference/.test(c)&&!/steel structure|structural steel|metalwork|supply and installation|machinery|equipment|civil works|building works/.test(c));if(consulting){sector='other';score=Math.min(score,25);reasons.push('consultancy/service only');}
...
export function parseUngm(html,source){const body=htmlToText(html),beneficiary=field(body,['Beneficiary countries or territories']);if(!beneficiary||!/\bkosovo\b/i.test(beneficiary))return null;const title=firstH1(html);if(!title)return null;const agency=String(html).match(/<h[2-4]\b[^>]*>\s*(UNDP|UNOPS|UNICEF|UNHCR|FAO|WHO|IOM|UN WOMEN|UNFPA|WFP|OSCE)\s*<\/h[2-4]>/i)?.[1];return{title,body,authority:clean(agency)||source.label,reference:field(body,['Reference']),published_date:dateAfter(body,['Published on','Published'])||null,deadline:dateAfter(body,['Deadline on','Deadline'])||null,notice_phase:phase(title),document_type:docType(title),detail_url:source.url};}
