...
const DOC='Direct Storage Dossier.docx';
const XLS='Direct Storage Prices.xlsx';
const EXPECTED=[DOC,XLS];
...
async function main(){
  ...
  const result={ok:false,version:'krpp-direct-storage-production-acceptance-v1',temp_tender_id:tempId,started_at:now,checks:{},cleanup:{storage_paths_removed:0,temp_queue_deleted:false,temp_tender_deleted:false,errors:[]}};
  ...
  try{
    ...
    result.checks.prepare_direct={upload_mode:prep.upload_mode,uploads:prep.uploads.map(x=>({expected_name:x.expected_name,source_name:x.source_name,path:x.path,has_signed_url:!!x.signed_url,has_token:!!x.token}))};
