...
export function planExecutionBootstrap({projects=[],wonOffers=[],supplierOffers=[],rfqs=[],existingTasks=[],today=new Date().toISOString().slice(0,10)}){
...
async function readState({supabaseUrl,apiKey}){
  const [projects,wonOffers,supplierOffers,rfqs,tasks]=await Promise.all([
  ...
  ]);return{projects,wonOffers,supplierOffers,rfqs,tasks};
}
...
export async function runWonExecutionBootstrap({
  ...
  const plan=planExecutionBootstrap({projects:state.projects,wonOffers:state.wonOffers,supplierOffers:state.supplierOffers,rfqs:state.rfqs,existingTasks:state.tasks,today});
