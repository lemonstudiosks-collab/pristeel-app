...
export function classifyTedNotice({title='',cpv=[]}={}){
  ...
  const rawCode=codes.find(c=>/^14622|^44171|^44172|^4433/.test(c));const structCode=codes.find(c=>/^4421/.test(c));
  if(rawCode){raw=86;reasons.push(`CPV lëndë çeliku: ${rawCode}`);}if(structCode){structure=90;reasons.push(`CPV strukturë çeliku: ${structCode}`);}
  if(/stahlbau|stahlkonstruk|stahltragwerk|metallbau|steelwork|structural steel|steel structure|staalbouw|staalconstruct/.test(n)){structure=Math.max(structure,78);reasons.push('titull/sinjal strukturë çeliku');}
  if(/steel plate|steel sheet|stahlblech|blech|profile|profil|bar|rod|wire|stahl|steel/.test(n)){raw=Math.max(raw,68);reasons.push('titull/sinjal lëndë çeliku');}
...
export function normalizeTedNotice(row,phase='opportunity',seenAt=new Date().toISOString()){
  const publication=firstScalar(field(row,'publication-number'));if(!publication)return null;const title=tedTitle(row)||`TED ${publication}`;const cpv=cpvCodes(row);const cls=classifyTedNotice({title,cpv});const type=firstScalar(field(row,'notice-type'));
  return{source_key:`TED:${publication}`,procurement_no:`TED-${publication}`,publication_no:publication,authority:buyer(row),title,document_type:type||null,fpp:cpv[0]||null,fpp_description:cpv.length?`CPV ${cpv.join(', ')}`:null,contract_type:null,contract_value_band:null,procedure:null,estimated_value:null,currency:'EUR',deadline:isoDate(field(row,'deadline')),published_date:isoDate(field(row,'publication-date')),is_retender:false,category:cls.category,relevance_score:cls.relevance_score,match_reasons:cls.match_reasons,source_url:`https://ted.europa.eu/en/notice/${encodeURIComponent(publication)}/html`,detail_url:`https://ted.europa.eu/en/notice/-/detail/${encodeURIComponent(publication)}`,payload:{source:'TED',notice_phase:phase,country:country(row)||null,cpv,notice_type:type||null},last_seen_at:seenAt,updated_at:seenAt};
}
...
async function tedSearch({query,scope='ACTIVE',fetchImpl=fetch,maxPages=4,limit=250}){
  ...
  for(let page=1;page<=maxPages;page++){
    const response=await fetchImpl(TED_API,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({query,fields:FIELDS,page,limit,scope,checkQuerySyntax:true,paginationMode:'PAGE_NUMBER',onlyLatestVersions:true})});
    const raw=await response.text();if(!response.ok)throw new Error(`TED search HTTP ${response.status}: ${raw.slice(0,1200)}`);let json;try{json=JSON.parse(raw);}catch{throw new Error(`TED search returned non-JSON: ${raw.slice(0,500)}`);}
...
export async function runTedTenderSync({mode=process.env.SYNC_MODE||'preview',minScore=Number(process.env.TED_TENDER_MIN_SCORE||55),fetchImpl=fetch,supabaseUrl=process.env.SUPABASE_URL||DEFAULT_SUPABASE_URL,apiKey=process.env.SUPABASE_SECRET_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY||process.env.SUPABASE_SERVICE_KEY||'',bearerToken=''}={}){
  if(!['preview','apply'].includes(mode))throw new Error(`Unsupported SYNC_MODE: ${mode}`);const seenAt=new Date().toISOString();const opportunityRaw=await tedSearch({query:queryFor(OPPORTUNITY_TYPES),scope:'ACTIVE',fetchImpl});const awardRaw=await tedSearch({query:queryFor(AWARD_TYPES,{days:35}),scope:'ACTIVE',fetchImpl});const dedupe=new Map();
  for(const [phase,items] of [['opportunity',opportunityRaw],['award',awardRaw]])for(const item of items){const row=normalizeTedNotice(item,phase,seenAt);if(!row||row.relevance_score<minScore)continue;const prev=dedupe.get(row.source_key);if(!prev||phase==='opportunity')dedupe.set(row.source_key,row);}
  const rows=[...dedupe.values()];let authMode='not_needed';if(mode==='apply'&&rows.length){const access=apiKey?{supabaseUrl,apiKey,bearerToken:bearerToken||apiKey,authMode:'service_key'}:await resolveSupabaseWorkflowAccess({supabaseUrl});authMode=access.authMode;await upsertRows(access,rows);}
  const summary={mode,auth_mode:authMode,api:TED_API,opportunity_raw:opportunityRaw.length,award_raw:awardRaw.length,relevant_rows:rows.length,opportunities:rows.filter(r=>r.payload.notice_phase==='opportunity').length,awards:rows.filter(r=>r.payload.notice_phase==='award').length,minimum_score:minScore,queries:{opportunities:queryFor(OPPORTUNITY_TYPES),awards:queryFor(AWARD_TYPES,{days:35})},tenders:rows.map(r=>({publication_no:r.publication_no,title:r.title,authority:r.authority,category:r.category,relevance_score:r.relevance_score,phase:r.payload.notice_phase,cpv:r.payload.cpv,published_date:r.published_date,deadline:r.deadline}))};await writeSummary(summary);console.log(`TED tender sync ${mode}: opportunityRaw=${summary.opportunity_raw}, awardRaw=${summary.award_raw}, relevant=${summary.relevant_rows}, opportunities=${summary.opportunities}, awards=${summary.awards}.`);return summary;
}
