...
async function main(){
  ...
  if(!r.ok||!analysis||analysis.ok===false)throw new Error(`Analyzer HTTP ${r.status}: ${text(analysis?.message||analysis?.error||raw,1800)}`);
  ...
  if(snapshot?.file_mode!=='authenticated_protected_archive')throw new Error('Canonical snapshot is not the protected archive analysis.');
  if(snapshot?.dossier_complete!==true)throw new Error('Canonical dossier snapshot is not complete.');
  if(!['local_deterministic','openai'].includes(snapshot?.provider?.name))throw new Error(`Unexpected canonical provider: ${text(snapshot?.provider?.name,120)||'none'}`);
  ...
  if(Array.isArray(integrity.remaining_expected)&&integrity.remaining_expected.length)throw new Error('Expected-file gaps remain after canonical analysis.');
  ...
  if(!price||price.extraction_state!=='extracted')throw new Error(`Price Intelligence was not persisted: ${JSON.stringify(price)}`);
...
main().catch(error=>{
  console.error(JSON.stringify({ok:false,tender_id:TENDER_ID,error:text(error?.stack||error?.message||error,5000)},null,2));
  process.exit(1);
});
