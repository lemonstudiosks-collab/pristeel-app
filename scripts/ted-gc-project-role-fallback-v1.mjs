...
const VERSION='ted-gc-project-role-fallback-v1.2';
const text=(v,max=5000)=>String(v==null?'':v).replace(/\s+/g,' ').trim().slice(0,max);
const norm=v=>text(v,10000).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
...
function hasDirectSteelPackageSignal(row){
  const t=norm(row?.title);
  return /(?:\bsteel\b|\bstahl\b|\biron bridge\b|\bbridge\b|\bbruecke\b|\bbrucke\b|\bprefabricated structures?\b|\bsubstation\b|\bumspannwerk\b|\bschaltanlage\b|\btransformer station\b|\bpower line\b|\belectricity power line\b|\bhochspannungsleitung\b|\belectricity transmission pylons?\b|\btransmission pylons?\b|\bpylons?\b|\btransmission towers?\b|\blattice towers?\b|\b(?:110|132|220|400)\s*k?v\b|\boverhead line\b|\bgantry\b|\bportal frame\b|\bwarehouse hall\b|\blagerhalle\b|\bindustrial plant\b|\bindustrieanlage\b)/i.test(t);
}
...
export async function runTedGcProjectRoleFallbackV1({mode=process.env.SYNC_MODE||'preview',minScore=Number(process.env.TED_GC_MIN_SCORE||82),maxRows=Number(process.env.TED_GC_FALLBACK_MAX||20),supabaseUrl=process.env.SUPABASE_URL||DEFAULT_SUPABASE_URL}={}){
  ...
  const [rows,outreach]=await Promise.all([
    rest(access,'kek_tender_watch?select=*&relevance_score=gte.82&order=published_date.desc,relevance_score.desc&limit=2000'),
  ...
  const rawCandidates=array(rows).filter(r=>{const p=payload(r),w=winner(r),type=String(w.company_type||w.company_classification?.company_type||'unknown');return String(p.source||'').toUpperCase()==='TED'&&p.notice_phase==='award'&&p.discovery_lane==='gc_project_award'&&r.status!=='ignored'&&type==='unknown'&&names(w).length===1&&isConstructionDeliveryAward(r)&&hasDirectSteelPackageSignal(r);});
  const seenCompanies=new Set(),candidates=[];
  for(const r of rawCandidates){const key=norm(names(winner(r))[0]);if(!key||seenCompanies.has(key))continue;seenCompanies.add(key);candidates.push(r);if(candidates.length>=Math.max(1,maxRows))break;}
  const routed=[];
  for(const row of candidates){
    const precision=evaluateGcAwardPrecision(row,{minScore});
    if(!precision.relevant)continue;
    const p=payload(row),now=new Date().toISOString(),role={version:VERSION,type:'gc_epc',confidence:precision.score>=94?'high':'medium',basis:'ted_construction_award_direct_steel_signal',precision_version:PRECISION_VERSION,assigned_at:now};
    ...
    if(alreadyContacted){routed.push({id:row.id,company:names(winner(row))[0],skipped:'already_contacted'});continue;}
  ...
  const summary={mode,version:VERSION,auth_mode:access.authMode,raw_candidates:rawCandidates.length,candidates:candidates.length,routed:routed.length,with_draft:routed.filter(x=>x.type==='gc_project_outreach_draft').length,contact_research:routed.filter(x=>x.type==='gc_contact_research').length,already_contacted:routed.filter(x=>x.skipped==='already_contacted').length,rows:routed,generated_at:new Date().toISOString()};
  await writeSummary(summary);console.log(`TED GC project-role fallback ${mode}: raw=${summary.raw_candidates}, candidates=${summary.candidates}, routed=${summary.routed}, drafts=${summary.with_draft}.`);return summary;
}
