...
const VERSION='ted-gc-project-role-fallback-v1.0';
...
function names(w){return [...new Set([...(array(w.names)),w.name].map(x=>text(x,500)).filter(Boolean))];}
...
function bestEmail(row,outreach){
  const w=winner(row),o=outreach||{};
  if(text(o.contact_email,400))return text(o.contact_email,400);
  const ranked=w?.contact_ranking?.email||w?.ranked_contact?.email;
  if(text(ranked,400))return text(ranked,400);
  for(const org of array(w?.contact_enrichment?.organizations))for(const c of array(org?.contacts))if(c?.type==='email'&&text(c?.value,400))return text(c.value,400);
  return text(w.email||array(w.emails)[0],400);
}
async function upsertAction(access,row,email,precision,mode){
  ...
  const brief=email
    ?`${company} është award-holder për “${project}”${buyer?` për ${buyer}`:''}. Corporate type ende nuk është zgjidhur, por scope-i i award-it është GC/EPC/infrastructure me relevancë të verifikuar për PriSteel. Përgatit outreach specifik për projektin duke ofruar steel fabrication/subcontracting capacity. Draft vetëm; mos dërgo automatikisht.`
    :`${company} është award-holder për “${project}”. Scope-i i award-it është GC/EPC/infrastructure me relevancë të verifikuar për PriSteel, por nuk kemi ende procurement/project email të besueshëm. Gjej kontaktin para draftit.`;
  const body={tender_watch_id:row.id,project_id:row.project_id||null,action_key:key,action_type:type,route:'TED_GC',status:'draft_review',priority:Number(row.relevance_score||0)>=94?'larte':'mesatare',due_date:dueDate(row),target_company:company,target_email:email||null,subject_hint:type==='gc_project_outreach_draft'?`Fabrication support · ${project.slice(0,130)}`:`GC contact research · ${company}`,draft_brief:brief,payload:{engine_version:VERSION,routing_basis:'awarded_project_scope',award_role:'gc_epc',award_role_confidence:precision.score>=94?'high':'medium',corporate_company_type:'unknown',precision_version:PRECISION_VERSION,human_approval_required:true},updated_at:new Date().toISOString()};
  if(mode==='apply'){
...
export async function runTedGcProjectRoleFallbackV1({mode=process.env.SYNC_MODE||'preview',minScore=Number(process.env.TED_GC_MIN_SCORE||82),supabaseUrl=process.env.SUPABASE_URL||DEFAULT_SUPABASE_URL}={}){
  ...
  const candidates=array(rows).filter(r=>{const p=payload(r),w=winner(r),type=String(w.company_type||w.company_classification?.company_type||'unknown');return String(p.source||'').toUpperCase()==='TED'&&p.notice_phase==='award'&&p.discovery_lane==='gc_project_award'&&r.status!=='ignored'&&type==='unknown'&&names(w).length===1;});
  const routed=[];
  for(const row of candidates){
    const precision=evaluateGcAwardPrecision(row,{minScore});
    if(!precision.relevant)continue;
    const p=payload(row),now=new Date().toISOString(),role={version:VERSION,type:'gc_epc',confidence:precision.score>=94?'high':'medium',basis:'awarded_project_scope',precision_version:PRECISION_VERSION,assigned_at:now};
