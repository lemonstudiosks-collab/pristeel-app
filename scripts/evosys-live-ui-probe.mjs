...
const report={
  ...
  candidate_intercept: !!CANDIDATE_URL,
  auth_mode:'one-time-admin-generated-magiclink',
...
async function httpJson(url,options={}){
  ...
  if(!r.ok){
    const err=new Error(`HTTP ${r.status} for ${new URL(url).pathname}`);
...
function svcHeaders(extra={}){
  return {...extra,apikey:SERVICE_KEY,Authorization:`Bearer ${SERVICE_KEY}`};
}
...
async function createUserSession(operator){
  const link=await httpJson(`${SB_URL}/auth/v1/admin/generate_link`,{
    ...
    body:JSON.stringify({type:'magiclink',email:operator.email})
  });
  if(!link?.hashed_token)throw new Error('Supabase did not return a hashed magic-link token');
  const variants=[link.verification_type||'magiclink','email'];
  let last=null;
  for(const type of [...new Set(variants)]){
    try{
      const session=await httpJson(`${SB_URL}/auth/v1/verify`,{
        ...
        body:JSON.stringify({type,token_hash:link.hashed_token})
      });
      if(session?.access_token&&session?.refresh_token)return session;
      last=new Error('Verification response had no session');
    }catch(e){last=e;}
  }
  if(link.email_otp){
    try{
      const session=await httpJson(`${SB_URL}/auth/v1/verify`,{
        method:'POST',headers:{apikey:SERVICE_KEY,'Content-Type':'application/json'},
        body:JSON.stringify({type:'email',email:operator.email,token:link.email_otp})
      });
      if(session?.access_token&&session?.refresh_token)return session;
    }catch(e){last=e;}
  }
...
async function rows(pathname){
  const v=await httpJson(`${SB_URL}/rest/v1/${pathname}`,{headers:svcHeaders()});
...
let browser;
try{
  const operator=await selectOperator();
  report.operator_role=operator.role;
  ...
  await page.waitForFunction(()=>typeof window.pstOpenProjectWorkspace==='function',{timeout:60000});
  await page.evaluate(async id=>{const r=window.pstOpenProjectWorkspace(id);if(r&&typeof r.then==='function')await r;},PROJECT_ID);
  ...
  report.diagnostics=await page.evaluate(()=>{
    const p=document.getElementById('page-workspace-project');
    const body=document.getElementById('pst-pi-body');
    const buttons=[...document.querySelectorAll('[data-mso-open]')].map(b=>({
      text:(b.textContent||'').trim(),disabled:!!b.disabled,display:getComputedStyle(b).display,
      visibility:getComputedStyle(b).visibility,pointerEvents:getComputedStyle(b).pointerEvents,
      rect:(()=>{const r=b.getBoundingClientRect();return{x:r.x,y:r.y,w:r.width,h:r.height};})()
    }));
    const scripts=[...document.scripts].filter(s=>String(s.src).includes('manual-supplier-offer')).map(s=>s.src.split('?')[0]);
    const supplierCards=[...(body?body.querySelectorAll('.pst-csf-suppliers,.pf2-card,section'):[])].slice(0,30).map(x=>({
      cls:x.className||'',text:(x.textContent||'').replace(/\s+/g,' ').trim().slice(0,350)
    }));
    return {
      page_area:p?.getAttribute('data-pwf-area')||'',page_stage:p?.getAttribute('data-pwf-stage')||'',
      module_flag:!!window.__pstManualSupplierOfferV1,module_api:!!window.PSTManualSupplierOfferV1,
      button_count:buttons.length,buttons,scripts,supplierCards,
      body_text:(body?.textContent||'').replace(/\s+/g,' ').trim().slice(0,1800)
    };
  });
  report.console_errors=consoleErrors.slice(0,30);
  await page.screenshot({path:path.join(OUT,'01-evosys-offers-before-click.png'),fullPage:true});

  const btn=page.locator('[data-mso-open]').first();
  const count=await btn.count();
  report.checks.manual_action_present=count>0;
  if(!count)throw new Error('Live EVOSYS supplier-offer stage has no [data-mso-open] action');
  report.checks.manual_action_visible=await btn.isVisible();
  report.checks.manual_action_enabled=await btn.isEnabled();
  const box=await btn.boundingBox();
  if(box){
    report.hit_target=await page.evaluate(({x,y})=>{
      const el=document.elementFromPoint(x,y);return el?{tag:el.tagName,cls:el.className||'',text:(el.textContent||'').trim().slice(0,200)}:null;
    },{x:box.x+box.width/2,y:box.y+box.height/2});
  }

  await btn.click({timeout:10000});
  const modal=page.locator('#pst-mso-modal');
  ...
  if(modalVisible){
    ...
    await page.screenshot({path:path.join(OUT,'02-evosys-modal-open.png'),fullPage:true});
  }else{
    await page.screenshot({path:path.join(OUT,'02-evosys-after-click-no-modal.png'),fullPage:true});
    throw new Error('Live click did not open #pst-mso-modal');
  }

  const afterClick=await businessCounts();
  report.after_click=afterClick;
  ...
  await page.locator('[data-mso-close]').first().click();
  await page.waitForTimeout(300);
  const afterClose=await businessCounts();
  report.after_close=afterClose;
  ...
  if(e?.status)report.http_status=e.status;
}finally{
