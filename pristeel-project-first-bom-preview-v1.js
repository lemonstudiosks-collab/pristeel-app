...
function rows(){var a=api(),src=a&&a.source||{},g=a&&Array.isArray(a.groups)?a.groups:[];return g.map(function(x,i){return{pos:i+1,profile:x[0],dim:x[1],kg:num(x[2]),label:x[3]||'',grade:src.grade||'S235JR',std:x[0]==='PLATE'?'EN 10029':'EN 10025-2'};});}
...
function renderPreview(){
  ...
  x.innerHTML='<div class="pbp-h"><div><b>Preview BOM · Çeliku strukturor</b><span>'+esc(src.file||'Dokumenti')+' · '+rs.length+' grupe · ende pa ruajtur</span></div><div class="pbp-actions"><button type="button" data-pbp-close>Mbyll preview</button><button type="button" class="primary" data-pbp-save>Ruaj BOM</button></div></div>'
    +'<div class="pbp-wrap"><table class="pbp-table"><thead><tr><th>Pos.</th><th>Profili</th><th>Dimensioni</th><th>Përshkrimi</th><th>Grada</th><th>Standardi</th><th style="text-align:right">Kg</th></tr></thead><tbody>'
    +rs.map(function(r){return'<tr><td>'+r.pos+'</td><td><b>'+esc(r.profile)+'</b></td><td>'+esc(r.dim)+'</td><td>'+esc(r.label)+'</td><td>'+esc(r.grade)+'</td><td>'+esc(r.std)+'</td><td class="kg">'+r.kg.toLocaleString('de-DE',{minimumFractionDigits:2,maximumFractionDigits:2})+'</td></tr>';}).join('')
    +'</tbody></table></div><div class="pbp-foot"><span>Scope: vetëm profile + pllaka S235JR. Pa armaturë, sandwich panel, Alubond, OSB, izolime/finitura dhe pa +3% spojna sredstva.</span><b>'+total(rs).toLocaleString('de-DE',{minimumFractionDigits:2,maximumFractionDigits:2})+' kg</b></div><div class="pbp-status">Preview vetëm. Asgjë nuk është shkruar në databazë.</div>';
...
async function save(){
  var id=pid(),d=data(),rs=rows(),btn=E('pst-pf2-bom-preview')&&E('pst-pf2-bom-preview').querySelector('[data-pbp-save]'),st=E('pst-pf2-bom-preview')&&E('pst-pf2-bom-preview').querySelector('.pbp-status');
  ...
  try{
    ...
    var payload=rs.map(function(r){return{project_id:id,profile:r.profile,dim:r.dim,grade:r.grade,std:r.std,len_mm:0,pcs:1,kg:r.kg,surface:'none',cert:'EN 10204 3.1'};});
    await window.supaFetch('bom_items','POST',payload);
    var verify=await window.supaFetch('bom_items?project_id=eq.'+encodeURIComponent(id)+'&select=kg&limit=5000');verify=Array.isArray(verify)?verify:[];
    var vt=+verify.reduce(function(s,r){return s+num(r.kg);},0).toFixed(2);
    if(verify.length!==18||Math.abs(vt-25828.74)>=0.01)throw new Error('Verifikimi pas ruajtjes deshtoi: '+verify.length+' rreshta / '+vt+' kg.');
    if(st){st.className='pbp-status ok';st.textContent='✓ BOM u ruajt dhe u verifikua: 18 grupe · 25.828,74 kg.';}
    if(btn){btn.textContent='✓ U ruajt';btn.disabled=true;}
    try{if(window.PSTProjectDataIntegrity&&window.PSTProjectDataIntegrity.load){var fresh=await window.PSTProjectDataIntegrity.load(id);window.__pstIntegrityLastData=fresh;if(window.PSTProjectFirstV2&&window.PSTProjectFirstV2.render)window.PSTProjectFirstV2.render('bom');}}catch(e){}
  }catch(e){console.error('PF2 BOM save:',e);if(st){st.className='pbp-status err';st.textContent='BOM nuk u ruajt: '+(e.message||e);}if(btn){btn.disabled=false;btn.textContent='Ruaj BOM';}}
}
function intercept(e){var b=e.target&&e.target.closest?e.target.closest('#pst-doc-bom-pf2-open'):null;if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();renderPreview();}
