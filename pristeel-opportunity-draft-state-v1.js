 ...
 * Persists successful human-created Gmail draft state on the canonical tender row,
...
function draftState(r){var p=payload(r),d=p.outreach_draft||p.gmail_draft||null;return d&&typeof d==='object'&&S(d.status).toLowerCase()==='created'?d:null;}
...
function decorateCard(card,r){
  ...
  if(meta&&!badge){badge=document.createElement('span');badge.className='pst-pcw-draft-badge';badge.textContent='DRAFT EMAILI U KRIJUA';meta.appendChild(badge);}
  if(open)open.textContent='Draft i krijuar';
  card.title='Draft emaili është krijuar. Kliko për ta parë gjendjen dhe opsionet.';
  return true;
}
function decorateModal(id,r){
  ...
  if(d){
    ...
    var b=actions.querySelector('[data-pcw-ti="draft"]');if(b){b.textContent='Drafti ekziston · Hap Gmail';b.classList.add('pst-pcw-draft-existing');b.disabled=false;}
  }else{
...
function openExisting(id,d){
  ...
  var when=d&&d.created_at?formatWhen(d.created_at):'';
  if(typeof window.pstToast==='function')window.pstToast('Draft emaili ekziston'+(when?' · '+when:'')+'. Po hap Gmail Drafts.','ok');
  window.open('https://mail.google.com/mail/u/0/#drafts','_blank','noopener');return {existing:true,draft:d||{}};
}
