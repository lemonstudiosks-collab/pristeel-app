...
async function askAi(pid,question){
  ...
  var ctx=await context(pid),ids=allowedSources(ctx),s=session(pid);
  ...
  return ai.requestJson({messages:[{role:'system',content:system}].concat(historyMessages(s),[{role:'user',content:prompt}]),temperature:0.15,max_tokens:2600,response_format:{type:'json_object'}});
}
