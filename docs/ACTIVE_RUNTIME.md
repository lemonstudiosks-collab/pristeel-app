...
- Mobile Home is intentionally different from the desktop action surface: the fixed bottom navigation owns Opportunities, Projects, Partners, Finance and System, while Home stays an information dashboard with the PRISTEEL header, greeting, `Pyet PPPP`, live Prishtina weather/date, steel-market source shortcuts, useful local tools and external market resources. Project/opportunity counters, active-project cards and quick workflow buttons are deliberately absent.
- Mobile Home adds no Supabase reads/writes and no polling. Its bounded public reads are a 30-minute-cached Open-Meteo weather request and an on-demand 12-hour-cached ECB daily reference-rate request for the currency converter. Stale/sample steel numbers are not shown as current prices; product rows open the current market source.
- `pristeel-mobile-pin-unlock-v1.js` owns the trusted-device mobile re-entry gate. After first-time normal authentication and PIN setup, routine mobile re-entry — including access-token expiry — is PIN-only: the PIN unlock restores the remembered refresh-token session and lets the existing auth refresh path renew it. The normal PIN gate does not expose email/password fallback; explicit logout still clears the trusted-device PIN/session state.
