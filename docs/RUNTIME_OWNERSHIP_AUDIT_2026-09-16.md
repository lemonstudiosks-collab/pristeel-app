...
Further observer audit found three presentation self-loops: `pristeel-operator-flow-v1.js` rewrote unchanged navigation text on every document mutation; `pristeel-workspace-skeleton-v3.js` rewrote unchanged project tab labels on every child mutation; `pristeel-project-stability-v2.js` reassigned the same picker `display` style on every body style/class mutation. The latter picker search could even select `body` or `html` when walking ancestors. The fixes make these writes conditional and prohibit a document-root picker target. Idle-observer DOM tests now assert the absence of self-generated mutations after settling.
...
The Home automation-truth reconciler had another confirmed self-triggering path: its document-wide MutationObserver scheduled `decorateHome()` after every mutation, while `decorateHome()` itself removed and recreated project labels and the waiting lane. Each pass also fetched up to 3,000 projects, 4,000 documents, 7,000 emails, and 4,000 RFQ records. The isolated presentation fix removes that observer, relies on the existing canonical Home render event, preserves unchanged DOM nodes, and shares the four read-only query results for 30 seconds. It does not change waiting-row classification or any write/automation semantics. A regression test confirms repeated Home events neither rewrite the waiting lane nor repeat the large reads within that window.

The existing Daily action surfaces CI check failed even on the baseline because the Opportunities Daily renderer had a page-wide MutationObserver, while its own test explicitly requires bounded, event-driven rendering. The TED Sales link is already ensured by `ensureStructure()` and `render()`, so the redundant observer was removed. The existing `opportunities-daily-smoke.js` now passes locally.

The main test suite also exposed an outdated static assertion that required hiding legacy Opportunities controls unconditionally. That contradicted the new safe fallback: hide them only when the replacement map exists. The guard now enforces the conditional selector, matching the runtime regression test.
