...
function ensureStructure(){
  ...
  if(!focus){focus=document.createElement('section');focus.id='pst-finance-focus';focus.innerHTML='<header><div><span>FINANCA</span><h1>Kërkon vëmendje</h1><p>Vetëm pagesat, faturat dhe verifikimet që kërkojnë ndërhyrje njerëzore.</p></div><button type="button" data-pst-fin-home>← Ballina</button></header><div id="pst-finance-work-list"><div class="pst-fin-loading">Duke lexuar punën financiare…</div></div>';hub.insertBefore(focus,grid);}
  var tools=hub.querySelector('#pst-finance-tools');
  if(!tools){tools=document.createElement('details');tools.id='pst-finance-tools';tools.open=true;tools.innerHTML='<summary><div><b>Harta e financave</b><span>Zgjidh një degë për të hapur regjistrin përkatës</span></div><i>Fshih / Hap</i></summary><div class="pst-fin-tools-body"></div>';var body=tools.querySelector('.pst-fin-tools-body');hub.insertBefore(tools,grid);body.appendChild(grid);}
  return focus;
}
...
function click(e){if(!active())return;var home=e.target&&e.target.closest?e.target.closest('[data-pst-fin-home]'):null;if(home){e.preventDefault();e.stopPropagation();var nav=window.PSTPrimaryNavResilienceV1;if(nav&&typeof nav.openHome==='function')nav.openHome();else if(typeof window.pstWorkspaceGo==='function')window.pstWorkspaceGo('home');return;}var b=e.target&&e.target.closest?e.target.closest('[data-pst-fin-open]'):null;if(!b)return;var id=b.getAttribute('data-pst-fin-open'),r=state.rows.filter(function(x){return S(x.id)===S(id);})[0];if(!r)return;e.preventDefault();e.stopPropagation();openRow(r);}
...
#page-finance.active #pst-finance-focus [data-pst-fin-home]{min-height:37px;padding:0 13px;border:1px solid #C9DDE2;border-radius:10px;background:#F3F9FA;color:#316D80;font-size:11px;font-weight:780;cursor:pointer}
