...
function intelligenceLabel(v){return({high:'Prioritet i lartë',medium:'Prioritet mesatar',low:'Prioritet i ulët',strong:'Përshtatje e fortë',possible:'Për shqyrtim',weak:'Përshtatje e dobët',direct_bid:'Ofertim i drejtpërdrejtë',winner_outreach:'Kontakt me fituesin',supplier_relation:'Marrëdhënie me furnitorin'})[v]||v;}
...
function ensureIntelligenceModal(){
 ...
 b.innerHTML='<div id="pst-ti-card" style="width:min(720px,96vw);max-height:88vh;overflow:auto;background:#fff;border-radius:14px;border:1px solid #D9DEE2;box-shadow:0 20px 70px rgba(0,0,0,.18);padding:20px"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px"><div><div style="font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#8A4E24">ANALIZA E MUNDËSISË</div><div id="pst-ti-title" style="font-size:17px;font-weight:750;color:#252A2E;margin-top:3px"></div><div id="pst-ti-meta" style="font-size:10.5px;color:#7A8187;margin-top:4px"></div></div><button type="button" onclick="pstTenderIntelligenceClose()" style="border:1px solid #D9DEE2;background:#fff;border-radius:8px;padding:6px 9px;cursor:pointer">Mbyll</button></div><div id="pst-ti-body" style="margin-top:16px"></div><div style="margin-top:16px;padding-top:10px;border-top:1px solid #ECEFF1;font-size:9.5px;color:#8A9095">Mbështetje për vendim · nuk krijon projekt, nuk ndryshon status dhe nuk dërgon email pa veprimin tënd.</div></div>';
...
window.pstTenderIntelligence=async function(id){
 ...
 var b=ensureIntelligenceModal();document.getElementById('pst-ti-title').textContent=r.title||'Tender';document.getElementById('pst-ti-meta').textContent=sourceLabel(r)+' · Përshtatja '+capabilityFitLabel(capabilityFit(r))+' · '+String(r.relevance_score||0)+'%';document.getElementById('pst-ti-body').innerHTML='<div style="font-size:12px;color:#6F7780">Duke përgatitur brief-in…</div>';b.style.display='flex';
...
function ensureFitFilter(){
 ...
 var fit=document.createElement('select');fit.id='pst-kek-fit';fit.innerHTML='<option value="all">Të gjitha përshtatjet</option><option value="strong">Përshtatje e fortë</option><option value="possible">Për shqyrtim</option><option value="weak">Përshtatje e dobët</option>';fit.onchange=window.pstKekRender;
...
function tedActions(r){
 ...
 if(isOperationalFocus(r))a+='<button class="pst-kek-btn" onclick="pstTenderIntelligence(\''+esc(r.id)+'\')">Analizo</button>';
