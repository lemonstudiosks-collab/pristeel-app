...
function rememberGmailCreate(){var n=document.getElementById('pgi2-name'),c=document.getElementById('pgi2-client'),r=document.getElementById('pgi2-ref');gmailPending={name:String(n&&n.value||'').trim(),client:String(c&&c.value||'').trim(),ref:String(r&&r.value||'').trim(),at:Date.now()};}
async function findGmailCreated(){var q=gmailPending;if(!q||!q.name||Date.now()-q.at>15000||typeof window.supaFetch!=='function')return false;try{var rows=await window.supaFetch('projects?name=eq.'+enc(q.name)+'&order=created_at.desc&select=id,name,client,ref,drive_folder_id,created_at&limit=8');rows=Array.isArray(rows)?rows:[];var p=rows.filter(function(x){if(q.client&&String(x.client||'').trim()!==q.client)return false;if(q.ref&&String(x.ref||'').trim()!==q.ref)return false;return true;})[0]||rows[0];if(!p)return false;gmailPending=null;return ensure(p.id);}catch(e){return false;}}
document.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('#pgi2-create');if(!b)return;rememberGmailCreate();[600,1300,2600,5000].forEach(function(ms){setTimeout(findGmailCreated,ms);});},true);
