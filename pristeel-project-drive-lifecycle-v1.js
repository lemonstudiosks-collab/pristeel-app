...
function wrapLegacySave(){var base=window.saveProject;if(typeof base!=='function'||base.__pstDriveLifecycle)return false;function wrapped(){var before=String(window._curProjId||'');var out=base.apply(this,arguments);[350,900,1800,3500,6000].forEach(function(ms){setTimeout(function(){var after=String(window._curProjId||'');if(after&&after!==before)ensure(after);},ms);});return out;}wrapped.__pstDriveLifecycle=true;wrapped.__base=base;window.saveProject=wrapped;return true;}
...
document.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('#pgi2-create');if(!b)return;var q=rememberGmailCreate();if(q.hasFiles)return;[600,1300,2600,5000,8000].forEach(function(ms){setTimeout(findGmailCreated,ms);});},true);
