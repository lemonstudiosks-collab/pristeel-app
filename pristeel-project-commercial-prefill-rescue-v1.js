...
function apply(){var page=document.getElementById('page-oferta');var p=armed||id();if(!p||!page||!page.classList.contains('active'))return false;if(!fresh())return false;neutralizeFreshDefaults();var api=window.PSTProjectCommercialPrefillV1;if(api&&typeof api.prefill==='function'){try{return api.prefill(p)!==false;}catch(e){if(window.console)console.warn('Project commercial prefill rescue:',e);}}return false;}
...
window.PSTProjectCommercialPrefillRescueV1={apply:apply,schedule:schedule,neutralizeFreshDefaults:neutralizeFreshDefaults};
})();
