-- Keep an explicitly linked tender email exclusive to that tender. Recipient
-- and company-domain fallbacks remain available only for still-unlinked mail.
...
outgoing as (
  select
    ...
    case
      when pe.tender_watch_id=a.tender_watch_id then 'explicit_tender'
    ...
    row_number() over (
      ...
      order by
        case
          when pe.tender_watch_id=a.tender_watch_id then 0
          when lower(e.email)=a.target_email then 1
          else 2
        end,
  ...
  where pe.tender_watch_id=a.tender_watch_id
     or (
       pe.tender_watch_id is null
       and (
         lower(e.email)=a.target_email
         or (
           a.target_domain<>''
...
incoming as (
  select
    ...
    row_number() over (
      ...
      order by
        case
          when pe.tender_watch_id=a.tender_watch_id then 0
          when lower(coalesce(pe.from_email,''))=a.target_email then 1
          else 2
        end,
        pe.sent_at desc nulls last,
        pe.id desc
  ...
  where pe.tender_watch_id=a.tender_watch_id
     or (
       pe.tender_watch_id is null
       and (
         lower(coalesce(pe.from_email,''))=a.target_email
         or (
           a.target_domain<>''
