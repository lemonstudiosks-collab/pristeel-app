...
as $function$
  select jsonb_build_object(
    'bridge_version','chatgpt-command-v16',
    'purpose','Account-level PPPP bridge for live reads plus controlled approved writes. Project Resolution v2 provides read-only auditable project identity resolution. Situation Intelligence v1 adds read-only cross-project synthesis. Action Orchestrator v1 converts situations into ranked proposal-only next actions. Daily PriSteel Intelligence v1 provides the standard read-only current operating brief. Finance Intelligence v1 adds read-only finance synthesis over canonical invoices, expenses, bank guarantees and outgoing invoice milestones without changing Finance core calculations or executing financial actions.',
    ...
    'operator_shorthand',jsonb_build_object(
      ...
      'çfarë arkëtojmë / çfarë kemi për të paguar','Use public.pppp_chatgpt_finance_intelligence_v1(integer,integer). Distinguish canonical receivables from supplier invoices and expense records; missing due dates are data-quality risks, not overdue conclusions; do not net different currencies or infer cash-on-hand.',
      'kontrollo garancitë bankare','Use public.pppp_chatgpt_finance_intelligence_v1(integer,integer). Review expiry dates, project linkage and exposure only; never extend, release or otherwise change a guarantee automatically.',
    ...
    'global_instruction','Whenever the user mentions PPPP or says “regjistroje në PPPP”, call public.pppp_chatgpt_bridge_manifest_v1() first and follow the returned protocol. For the standard daily/current operating brief use public.pppp_chatgpt_daily_intelligence_v1(integer,integer). For Finance questions use public.pppp_chatgpt_finance_intelligence_v1(integer,integer). Finance Intelligence is read-only: do not modify Finance core calculations, do not execute payments, do not approve invoices, do not change bank guarantees, and do not decide final price or margin. Keep currencies separate unless a canonical conversion is explicitly available; do not treat outgoing milestones as receivables until invoiced; do not merge expense records into supplier invoices; missing due dates are not overdue conclusions; no cash balance may be inferred without a canonical bank balance source. For deeper cross-project operating-picture use public.pppp_chatgpt_situation_intelligence_v1(integer,integer). For ranked next-step proposals use public.pppp_chatgpt_action_orchestrator_v1(integer,integer). Action Orchestrator v1 is proposal-only and performs zero executions. Any controlled write requires explicit user approval and the existing bridge protocol; protected actions remain human gated. For one resolved project, use public.pppp_chatgpt_project_situation_intelligence_v1(uuid,integer). Project Memory and canonical operational state have precedence, current operator actions drive urgency, raw workflow-task backlog does not drive urgency, unresolved inbox items are never forced into projects, and missing reply evidence alone never creates action. For project identity questions use public.pppp_chatgpt_project_resolution_v2(text,text,jsonb,integer). Preserve external email, supplier selection/commitment, final selling price/margin, contract/PO and won/lost human gates.'
  );
$function$;