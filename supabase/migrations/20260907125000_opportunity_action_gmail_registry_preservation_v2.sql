-- Preserve the complete externally-created Gmail draft registry across idempotent Opportunity Engine refreshes.
-- This extends v1 beyond the legacy single-draft identifiers so a refresh cannot orphan multi-contact drafts.
...
declare
  ...
  keys text[] := array[
    ...
    'gmail_draft_generator',
    'gmail_draft_generator_target',
    'gmail_draft_generator_complete',
    'gmail_drafts',
    'gmail_draft_count',
    'gmail_recipient_count',
    'gmail_recipients',
    'gmail_auto_send',
    'human_send_required'
  ];
...
-- Recreate explicitly so deployments are deterministic even if the original trigger was altered.
