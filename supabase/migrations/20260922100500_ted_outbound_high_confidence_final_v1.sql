-- Canonical final TED outbound high-confidence gates.
-- Consolidates the production functions introduced during the 2026-09-22 hardening.
-- This migration does not enable sending, approve any queue row, or alter historical delivery state.
...
CREATE OR REPLACE FUNCTION public.pppp_outbound_approve_rows_v1(p_queue_ids uuid[], p_approved_by text, p_approval_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
