-- Keep document-intake within Edge memory limits and route oversized sources to human review.
...
begin
  if v_action='run' then
    ...
    select pal.project_id::uuid,
           ...
           'PPPP e ndaloi përpunimin automatik sepse skedari kalon kufirin e sigurt të memories për Edge Function. Burimi ruhet në Gmail; kërkohet përpunim lokal/manual ose konvertim. Attachment link #'||pal.id::text,
    ...
    set analysis_status='review',
        analysis_method='resource-guard-v1',
...
select cron.alter_job(job_id := 16, command := 'select public.project_document_intake_internal_request(''run'',3);');
