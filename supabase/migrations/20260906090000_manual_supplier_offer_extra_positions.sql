-- Extend the existing manual supplier-offer payload with optional extra cost positions.
-- No table/schema shape changes: public.offers.positions already stores JSONB.
-- Existing callers remain compatible because the RPC signature stays (uuid, jsonb).
-- Supplier selection remains a separate human-gated RPC.

create or replace function public.pppp_create_manual_supplier_offer_v1(
  p_project_id uuid,
  p_payload jsonb
)
returns jsonb
...
declare
  ...
  v_validity integer;
  v_positions jsonb;
  v_extra_input jsonb := coalesce(p_payload->'extra_positions','[]'::jsonb);
