...
as $function$
  select jsonb_build_object(
    'bridge_version','chatgpt-command-v13',
    'purpose','Account-level PPPP bridge for live reads plus controlled approved writes. Project Resolution v2 provides read-only auditable project identity resolution. Situation Intelligence v1 adds read-only cross-project synthesis over canonical project state, Project Memory, current operator actions, Business Inbox and Project Resolution without changing project assignments or executing actions. New projects, canonical supplier-offer registrations, and explicitly approved project dispositions from ChatGPT travel through the command sheet and trusted bridge worker because the ChatGPT database connector is read-only.',
    ...
    'operator_shorthand',jsonb_build_object(
      ...
      'çfarë po ndodh në PriSteel / ku duhet të fokusohem','Use public.pppp_chatgpt_situation_intelligence_v1(integer,integer) for the read-only cross-project operating picture. It combines project situations and unresolved inbox situations without executing actions.',
    ...
    'global_instruction','Whenever the user mentions PPPP or says “regjistroje në PPPP”, call public.pppp_chatgpt_bridge_manifest_v1() first and follow the returned protocol. For cross-project operating-picture, prioritization, or “what is happening” questions, use public.pppp_chatgpt_situation_intelligence_v1(integer,integer). For one resolved project, use public.pppp_chatgpt_project_situation_intelligence_v1(uuid,integer). Situation Intelligence is read-only and review-only: Project Memory and canonical operational state have precedence, current operator actions drive urgency, raw workflow-task backlog does not drive urgency, unresolved inbox items are never forced into projects, and no action is executed. For project identity questions, use public.pppp_chatgpt_project_resolution_v2(text,text,jsonb,integer): only Tier A may produce resolved, Tier B remains a review-only suggestion, Tier C/identity/company/domain/legacy suggested_project_id never assigns a project, multiple strong candidates are ambiguous, and closed projects require explicit/thread/project-specific strong evidence. For company/person/supplier/manufacturer questions, prefer public.pppp_chatgpt_entity_intelligence_v3(text,integer). For inbox/attention questions, prefer public.pppp_chatgpt_next_action_intelligence_v1(integer,integer) and use public.pppp_chatgpt_business_inbox_project_resolution_v1(integer,integer) when project mapping is needed. Missing reply evidence alone never means action required. Rejection/outcome evidence is review-only and never auto-closes a project. Preserve all protected human approval gates.'
  );
$function$;
