-- PPPP V2 / EU Award Sales
-- A TED award becomes a canonical Project only after an authenticated operator
-- presses the explicit approval action. Discovery, contact research and draft
-- generation remain non-committing; outbound email still requires human send.
...
begin
  ...
  -- Materialize verified outreach contacts in the existing Project contact store.
  for v_contact in
...
-- Future TED messages inherit the Project only after the tender itself has been
-- explicitly linked by the approval RPC above.
create or replace function public.pppp_link_ted_email_to_approved_project_v1()
returns trigger
language plpgsql
security invoker
set search_path to 'pg_catalog', 'public'
as $function$
begin
  if new.project_id is null and new.tender_watch_id is not null then
    select k.project_id into new.project_id
    from public.kek_tender_watch k
    where k.id = new.tender_watch_id
...
revoke all on function public.pppp_link_ted_email_to_approved_project_v1()
  from public, anon, authenticated;
grant execute on function public.pppp_link_ted_email_to_approved_project_v1()
  to service_role, postgres;

drop trigger if exists trg_pppp_link_ted_email_to_approved_project_v1
  on public.project_emails;
create trigger trg_pppp_link_ted_email_to_approved_project_v1
before insert or update of tender_watch_id on public.project_emails
for each row
execute function public.pppp_link_ted_email_to_approved_project_v1();
