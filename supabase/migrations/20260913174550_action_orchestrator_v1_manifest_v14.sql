...
as $function$
  select jsonb_build_object(
    'bridge_version','chatgpt-command-v14',
    'purpose','Account-level PPPP bridge for live reads plus controlled approved writes. Project Resolution v2 provides read-only auditable project identity resolution. Situation Intelligence v1 adds read-only cross-project synthesis. Action Orchestrator v1 converts those situations into ranked proposal-only next actions, draft/artifact preparation candidates, controlled-write candidates and explicit human gates without executing anything automatically.',
    ...
    'operator_shorthand',jsonb_build_object(
      ...
      'çfarë po ndodh në PriSteel / ku duhet të fokusohem','Use public.pppp_chatgpt_situation_intelligence_v1(integer,integer) for the read-only operating picture, then public.pppp_chatgpt_action_orchestrator_v1(integer,integer) when ranked proposed next actions are useful.',
