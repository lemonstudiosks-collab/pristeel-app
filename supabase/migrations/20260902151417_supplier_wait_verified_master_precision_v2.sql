...
set kind='client',
    ...
    person='Aleksandar Cingelic',
    notes=case when coalesce(notes,'') ilike '%PPPP supplier-role correction%' then notes else concat_ws(E'\n',nullif(notes,''),'PPPP supplier-role correction: repeated project/email evidence identifies this contact as an ITALIAN STYLE client contact, not a supplier.') end
where lower(coalesce(email,''))='aleksandarcinge@gmail.com';

update public.tasks t
set status='mbyllur',
    done_at=coalesce(done_at,now()),
    detail=case when coalesce(detail,'') ilike '%supplier master verification%' then detail else concat_ws(E'\n',nullif(detail,''),'PPPP: auto-closed after supplier master verification; contact is not a verified active supplier.') end
where t.source='supplier_wait_auto'
  ...
  and not exists (
    ...
    where lower(coalesce(pc.email,''))=lower(coalesce(t.contact_email,''))
      and sp.stage='active'
