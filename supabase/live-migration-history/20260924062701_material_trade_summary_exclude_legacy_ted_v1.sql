...
with active as (
  ...
  where t.target_status not in ('closed','rejected')
    and t.source_key like 'eu:%'
),
hot as (
  select
    id, company_name, project_title, quote_readiness, why_now,
    estimated_tonnes, procurement_timing
  from active
  order by
...
select
  ...
  (select count(*)::int from active
    where has_resolved_contact
      and outreach_status in ('not_ready','ready','queued')
      and project_id is null) as ready_for_outreach,
