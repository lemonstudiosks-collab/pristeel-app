...
const GENERATOR='pppp-opportunity-draft-generator-v8-contact-attribution-routing';
...
async function processAction(a:any,budget:{writes:number}){let p=a.payload&&typeof a.payload==='object'?a.payload:{},tender=await tenderContext(a.tender_watch_id);const recipients=/^TED_/i.test(text(a.route,80))?resolveTedRecipients(a,tender,MAX_CONTACTS_PER_ACTION):resolveTedRecipients(a,{winner:{email:a.target_email}},1);if(!recipients.length)return{action_key:a.action_key,event:'no_recipients',recipients:0,created:0,preserved:0,sent:0,remaining:0};await seedLegacyTenderDraft(a,tender,recipients);let created=0,preserved=0,sent=0,failures:any[]=[];
