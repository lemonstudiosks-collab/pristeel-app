...
const GENERATOR='pppp-opportunity-draft-generator-v6-delivery-registry';
...
async function findSent(row:any){const tk=await token(),u=new URL(`https://gmail.googleapis.com/gmail/v1/users/${encodeURIComponent(GMAIL_USER)}/messages`);u.searchParams.set('q',`in:sent rfc822msgid:${row.rfc_message_id}`);u.searchParams.set('maxResults','5');const r=await fetch(u,{headers:{Authorization:`Bearer ${tk}`}}),raw=await r.text();if(!r.ok)throw new Error(`Gmail sent search ${r.status}: ${raw.slice(0,500)}`);const data=JSON.parse(raw||'{}');for(const m of data.messages||[]){const full=await gmailMessage(m.id),labels=new Set((full?.labelIds||[]).map(String));if(!labels.has('SENT'))continue;const x=hdr(full,'X-PPPP-Outreach-ID'),mid=hdr(full,'Message-ID'),aid=hdr(full,'X-PPPP-Action-ID');if(x===row.outreach_id||mid===row.rfc_message_id||(aid===row.action_id&&mid===row.rfc_message_id))return full;}return null;}
...
async function markSent(row:any,meta:any){const now=new Date().toISOString(),at=sentAt(meta),method=hdr(meta,'X-PPPP-Outreach-ID')===row.outreach_id?'x-pppp-outreach-id':'rfc822-message-id';const {data,error}=await db.from(REGISTRY).update({status:'sent',gmail_message_id:meta.id,gmail_thread_id:meta.threadId||null,sent_at:at,last_checked_at:now,last_error:null,updated_at:now,payload:{...(row.payload||{}),sent_match_method:method}}).eq('id',row.id).select('*').single();if(error)throw error;return data;}
