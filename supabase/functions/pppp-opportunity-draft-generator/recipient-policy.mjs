...
export function isBlockedOutreachEmail(v){const e=normalizeEmail(v),local=e.split('@')[0]||'';return BLOCKED_OUTREACH_LOCAL_PARTS.has(local)||/^20[a-z]{2,}\.[a-z]{2,}$/.test(local);}
...
export function recipientGreeting(company,recipient){const kind=recipientKind(recipient),n=kind==='general'?'':explicitName(recipient?.name);return n?`Dear ${n},`:'Dear Sir or Madam,';}
