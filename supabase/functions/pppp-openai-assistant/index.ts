...
function projectScore(question:string,p:any){const n=N(question),words=new Set(n.split(' ').filter(Boolean));let s=0;const stop=new Set(['projekt','projekti','project','tender','ofert','oferta','client','klient','gmbh','shpk','bau','pristeel']);const vals=[p?.project,p?.name,p?.client,p?.ref,p?.business_ref].map(x=>N(x)).filter(Boolean);for(const x of vals){if(x!=='pristeel'&&n.includes(x))s=Math.max(s,100+x.length);for(const w of x.split(' ')){if(w.length>=3&&!stop.has(w)&&words.has(w))s=Math.max(s,20+w.length)}}return s}
...
function deterministicOpportunity(ctx:any){
  const tender=ctx?.tender&&typeof ctx.tender==='object'?ctx.tender:{},payload=tender?.payload&&typeof tender.payload==='object'?tender.payload:{},winner=payload?.winner&&typeof payload.winner==='object'?payload.winner:{};
  const name=T(winner?.name||'fituesi i publikuar',240),role=N(ctx?.winner_role||winner?.company_type||'unknown'),score=Number(tender?.relevance_score||0),emails=[T(winner?.email,320),...(Array.isArray(winner?.emails)?winner.emails.map((x:any)=>T(x,320)):[])].filter(Boolean),hasEmail=emails.length>0;
  const lines=[] as string[];
  if(score>0)lines.push(`Kjo opportunity TED është ${Math.round(score)}% relevante për PRISTEEL sipas filtrit të relevancës së tenderit.`);
  if(role==='gc_epc')lines.push(`${name} është klasifikuar si GC / EPC; qasja e arsyeshme është steel fabrication ose subcontracting për paketën konkrete.`);
  else if(role==='producer')lines.push(`${name} është klasifikuar si prodhues / konkurrent; qasja duhet kufizuar te overflow fabrication, kapacitet shtesë ose ndarje pakete.`);
  else if(role==='trader_consortium')lines.push(`${name} është klasifikuar si trader / konsorcium; duhet verifikuar roli konkret para outreach.`);
  else lines.push(`Roli i ${name} nuk është ende i verifikuar. Relevanca e tenderit nuk duhet interpretuar si verifikim i rolit të kompanisë.`);
  if(!hasEmail)lines.push('PPPP nuk ka email të verifikueshëm për këtë fitues; nuk ka bazë për status “Në pritje” ose “Dërguar”.');
  const next=!hasEmail?'Gjej dhe verifiko një kontakt të fituesit para përgatitjes së outreach.':role==='unknown'?'Verifiko rolin e fituesit para se të përgatitet outreach.':'Shqyrto kontaktin dhe përgatit draft vetëm nëse qasja komerciale është e përshtatshme; dërgimi mbetet vendim i njeriut.';
  return{ok:true,answer:lines.join('\n'),confidence:role!=='unknown'&&hasEmail?'high':'medium',uncertainty:role==='unknown'?'Roli i fituesit nuk është verifikuar.':(!hasEmail?'Mungon email i verifikueshëm i fituesit.':''),suggested_next_step:next,navigation:{project_id:null,project_name:null,area:null},evidence:[{source:'TED opportunity',reason:T(tender?.title||tender?.procurement_no||tender?.publication_no,500)},{source:'Fituesi TED',reason:name+(hasEmail?' · kontakt i disponueshëm':' · pa email të verifikuar')}],provider:{name:'pppp-opportunity-context',model:'deterministic-v1'},read_only:true};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(body?.context?.tender&&T(body?.scope,30)!=='project')return J(deterministicOpportunity(body.context));
