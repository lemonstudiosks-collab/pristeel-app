...
async function run(limit=10){
  ...
  const candidates=(data||[]).filter((x:any)=>{if(!/(plan|dinamik|schedule|timeline)/i.test(s(x.attachment_name)))return false;const needsAnalysis=s(x.analysis_method)!=='xlsx-dynamic-plan-v1'&&s(x.extracted_data?.document_type)!=='dynamic_plan';const needsArchive=!s(x.storage_path);return needsAnalysis||needsArchive;}).slice(0,Math.max(1,Math.min(20,Number(limit)||10)));
  const items=[];for(const x of candidates){try{items.push(await processOne(x));}catch(e){items.push({id:x.id,name:x.attachment_name,error:String(e)});}}
