...
function extractRequest(row:any){
  ...
  return{urgent,needs,positions:extractPositions(raw),mass:extractMass(raw)};
}
...
async function ensureTask(row:any,project:any,r:any){const due=new Date();if(!r.urgent)due.setUTCDate(due.getUTCDate()+1);const patch={project_id:project.id,title:taskTitle(project,r),detail:taskDetail(row,r),due_date:due.toISOString().slice(0,10),priority:r.urgent?"e larte":"mesatare",contact_email:row.from_email||null,category:"klient"};const {data}=await db.from("tasks").select("id,status").eq("source","email_request_auto").eq("source_ref",row.gmail_message_id).limit(1);if(data?.length){const {error}=await db.from("tasks").update(patch).eq("id",data[0].id);if(error)throw error;return "updated";}const {error}=await db.from("tasks").insert({...patch,status:"hapur",source:"email_request_auto",source_ref:row.gmail_message_id});if(error)throw error;return "created";}
...
async function reconcile(days=2,limit=300){
  ...
  const summary:any={checked:(rows||[]).length,linked:0,review:0,tasks_created:0,tasks_updated:0,unmatched:0,items:[]};
  for(const row of rows||[]){const d=classify(row,index);if(d.kind==="auto"){
      ...
      const req=extractRequest(row);let taskAction="";if(req)taskAction=await ensureTask(row,d.project,req);if(taskAction==="created")summary.tasks_created++;if(taskAction==="updated")summary.tasks_updated++;
      summary.items.push({gmail_message_id:row.gmail_message_id,project_id:d.project.id,project_name:d.project.name,score:d.score,reason:d.reason,task_action:taskAction||null,request:req?{urgent:req.urgent,needs:req.needs,positions:req.positions,mass:req.mass}:null});
