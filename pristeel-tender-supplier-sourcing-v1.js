...
function state(c,external){
 if(external)return c.contact_ready?{label:'KONTAKT GATI · VERIFIKO',cls:'review'}:{label:'VERIFIKO',cls:'review'};
...
function supplierCard(title,r,c,external){
 var st=state(c,external),meta=[];if(c.country)meta.push(c.country);if(c.source_tier)meta.push(c.source_tier);if(!external&&c.match_score!=null)meta.push('match '+c.match_score);if(external&&c.standard_evidence)meta.push('standard i gjetur');if(external&&c.certificate_evidence)meta.push('certifikim i gjetur');
 var actions='';if(c.website)actions+='<a target="_blank" rel="noopener" href="'+E(c.website)+'">Website ↗</a>';if(c.email)actions+='<a target="_blank" rel="noopener" href="'+E(gmail(title,r,c))+'">RFQ draft</a>';
...
function reqHtml(title,r){
 var i=r.internal||{},strict=Number(i.strict_rfq_ready_existing||0),review=Number(i.review_rfq_ready_existing||0),ext=r.external||null,external=A(ext&&ext.candidates),internal=A(i.candidates).filter(function(c){return c.strict_fit||c.review_fit||c.evidence_gap;}).slice(0,4),cards=internal.map(function(c){return supplierCard(title,r,c,false);}).join('')+external.slice(0,6).map(function(c){return supplierCard(title,r,c,true);}).join('');
 var st=strict>=3?{t:strict+'/3 RFQ-ready',c:'ready'}:(strict+review?{t:strict+'/3 gati · '+review+' për verifikim',c:'review'}:{t:'0/3 RFQ-ready',c:'find'});
 var tagHtml=tags(r).map(function(x){return'<span>'+E(x)+'</span>';}).join(''),externalNote=ext?'<div class="pts-empty">Kërkimi publik u bë vetëm për këtë artikull. U gjetën '+Number(ext.contact_ready_count||0)+' kontakte me email që kërkojnë verifikim teknik.</div>':'';
 ...
 +externalNote+'<div class="pts-actions"><button type="button" class="primary" data-pts-discover="'+E(r.id)+'">'+E(ext?'Kërko përsëri në web':'Gjej furnitorë')+'</button></div></article>';
}
