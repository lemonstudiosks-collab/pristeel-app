 ...
 * Confirmed-thread continuity repair runs only after this gate has a cached Gmail+Drive token.
 */
(function(){
'use strict';
...
async function resume(pid,target){
  var token=cached(),C=window.PSTProjectIntakeContinuityV1;
  if(C&&typeof C.normalizeProjectThreads==='function'){
    try{await C.normalizeProjectThreads(pid,token);}catch(e){if(window.console&&console.warn)console.warn('PRISTEEL intake continuity:',e);}
  }
  return target(pid);
}
function show(id,target){
  ...
  document.getElementById('pst-linked-gmail-auth-run').onclick=async function(){var b=this;try{b.disabled=true;b.textContent='Duke autorizuar…';setMsg('Po hapet Google…','');await authorize();setMsg('Autorizimi përfundoi. Duke vazhduar…','');remove();await resume(pid,target);}catch(e){b.disabled=false;b.textContent='Autorizo Gmail & Drive';setMsg(String(e&&e.message||e),'err');}};
  return false;
}
function install(){
  var target=window.pstRecoverLinkedProjectGmail;
  if(typeof target!=='function'||target.__pstLinkedGmailAuthGate)return false;
  function gated(id){var pid=projectId(id);if(cached())return resume(pid,target);return show(pid,target);}
...
window.PSTLinkedGmailAuthGateV1={install:install,show:show,cached:cached,resume:resume};
})();