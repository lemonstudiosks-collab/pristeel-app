...
function trimChrome(){var h=document.querySelector('#page-workspace-projects .pst-pm-head');if(h){var t=h.querySelector('.pst-pm-title'),s=h.querySelector('.pst-pm-sub');if(t)t.textContent='Projektet';if(s)s.textContent='Një pamje e qartë e punës, afateve dhe hapit të ardhshëm.';}var c=document.querySelector('#page-workspace-contacts .pcm-head p');if(c)c.textContent='Klientët, furnitorët dhe kontaktet — të lidhur me punën reale.';}
...
document.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('#pst-ws-canonical-nav,#fin-hub-grid,[data-pn-area],[data-pn-project-id],[data-pn-action-id]'))[0,180,650].forEach(function(ms){setTimeout(apply,ms);});},true);
