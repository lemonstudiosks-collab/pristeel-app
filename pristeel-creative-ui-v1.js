...
#page-workspace-projects .pst-pm-eyebrow{display:none!important}
...
#page-finance #fin-hub-grid.pst-finance-map{position:relative;display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:18px 12px!important;padding:20px!important;border:1px solid #DBE7E6;border-radius:23px;background:linear-gradient(135deg,#FBFDFC,#F3F8F8)!important}
#page-finance .pst-finance-map-core{grid-column:1/-1;justify-self:center;z-index:2;min-width:220px;margin:0 0 37px;padding:16px 25px;border-radius:19px;background:#315F74;color:#FFF;text-align:center;box-shadow:0 13px 29px rgba(49,95,116,.18)}
.pst-finance-map-core span{color:#C8E3E9;margin-right:8px}.pst-finance-map-core strong{font-size:18px}.pst-finance-map-core small{display:block;margin-top:4px;color:#D3E7E9;font-size:10px}
#page-finance #fin-hub-grid.pst-finance-map:before{content:"";position:absolute;left:12%;right:12%;top:116px;height:30px;border-top:1px solid #BFD5D9;border-left:1px solid #BFD5D9;border-right:1px solid #BFD5D9;border-radius:15px 15px 0 0;pointer-events:none}
#page-finance #fin-hub-grid.pst-finance-map>.pst-creative-fin-tile{position:relative;min-height:118px!important;padding:16px!important;border-radius:15px!important;box-shadow:0 5px 15px rgba(39,55,61,.045)!important}
#page-finance #fin-hub-grid.pst-finance-map>.pst-creative-fin-tile:before{content:"";position:absolute;left:50%;top:-19px;width:1px;height:18px;background:#BFD5D9;pointer-events:none}
#page-finance #fin-hub-grid.pst-finance-map .pst-creative-fin-icon{width:31px!important;height:31px!important;margin-bottom:9px!important;border-radius:9px!important}
#page-finance #fin-hub-grid.pst-finance-map .pst-creative-fin-icon svg{width:17px;height:17px}
#page-finance #fin-hub-grid.pst-finance-map>.pst-creative-fin-tile:hover{transform:translateY(-2px)!important}
...
@media(max-width:1180px){#pst-native-home-v4 .pn-cockpit{grid-template-columns:1fr!important}#pst-native-home-v4 .pn-side{position:static!important;grid-template-columns:1fr 1fr!important}#page-finance #fin-hub-grid,#page-finance #fin-hub-grid.pst-finance-map{grid-template-columns:repeat(2,minmax(0,1fr))!important}}
@media(max-width:820px){#pst-ws-sidebar{width:100%!important}#pst-native-home-v4{padding:15px!important}#pst-native-home-v4 .pn-head{padding:19px!important}#pst-native-home-v4 .pn-head:before,#pst-native-home-v4 .pn-head:after{display:none}#pst-native-home-v4 .pn-flow>.pn-work-grid{grid-template-columns:1fr!important}#pst-native-home-v4 .pn-lane{border-right:0!important;border-bottom:1px solid #E8EDEB!important}#pst-native-home-v4 .pn-lane-wait{grid-column:auto!important}#pst-native-home-v4 .pn-lane-wait .pn-work-list{grid-template-columns:1fr!important}#pst-native-home-v4 .pn-side{grid-template-columns:1fr!important}#page-finance #fin-hub-grid,#page-finance #fin-hub-grid.pst-finance-map{grid-template-columns:1fr!important}#page-finance #fin-hub-grid.pst-finance-map:before,#page-finance #fin-hub-grid.pst-finance-map>.pst-creative-fin-tile:before{display:none}}
...
function decorateFinance(){var grid=document.getElementById('fin-hub-grid');if(!grid)return false;grid.classList.add('pst-finance-map');var core=grid.querySelector('.pst-finance-map-core');if(!core){core=document.createElement('div');core.className='pst-finance-map-core';core.innerHTML='<span aria-hidden="true">✦</span><strong>Financat</strong><small>Zgjidh një degë për të hapur regjistrin</small>';grid.prepend(core);}Array.prototype.slice.call(grid.children).filter(function(tile){return tile!==core;}).forEach(function(tile,i){tile.classList.add('pst-creative-fin-tile');var icon=Array.prototype.slice.call(tile.children).filter(function(n){return n.tagName==='DIV'&&/font-size:\s*22px/i.test(n.getAttribute('style')||'');})[0];if(icon){icon.classList.add('pst-creative-fin-icon');icon.innerHTML=ICONS[i]||ICONS[0];}});return true;}
...
/* Navigation and Finance owners already refresh their own surfaces. Reapplying
   the entire visual layer five times after every click caused late layout jumps. */
document.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('#fin-hub-grid'))requestAnimationFrame(decorateFinance);},true);
