...
(function(){
'use strict';
if(window.__pstStartupGuardV2)return;
...
style.id='pst-startup-critical-css-v2';
style.textContent=`
:root{
...
.pst-startup-retry{display:none;margin-top:5px;border:1px solid #CFE0E6;border-radius:9px;background:#fff;color:#3F7F98;padding:8px 12px;font-size:10.5px;font-weight:700;cursor:pointer}
.pst-startup-stalled .pst-startup-line{display:none}.pst-startup-stalled .pst-startup-retry{display:inline-flex}
...
html.pst-app-ready #app-shell-root{opacity:1!important;visibility:visible!important;pointer-events:auto!important;animation:pstAppReveal .22s ease both}
...
function check(){
  ...
  ensureShell();polishLogin();
  var gate=document.getElementById('auth-gate'),app=document.getElementById('app-shell-root');
  var gateOn=intendedVisible(gate),appOn=intendedVisible(app);
  if(gateOn&&!appOn){
    if(!sessionExists()||state.loaded){finish('auth');return;}
