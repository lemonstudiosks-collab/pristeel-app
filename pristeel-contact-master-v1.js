...
function renderDetail(r,pending){
 var m=ensureModal(),h=m.querySelector('#pcm-detail'),pc=projectCount(r),emails=Number(r.project_email_count||0);
 h.innerHTML='<span class="pcm-eyebrow">DOSJA E KONTAKTIT</span><h2>'+E(nameOf(r))+'</h2><p class="pcm-company">'+E([r.company,r.role,kindLabel(r.kind),isManufacturer(r)?'Prodhues':''].filter(Boolean).join(' · '))+'</p>'
  +'<div class="pcm-detail-grid"><section><span>Email</span><b>'+E(r.email||'—')+'</b></section><section><span>Telefon</span><b>'+E(r.phone||'—')+'</b></section><section><span>Projektet</span><b>'+pc+'</b><small>'+emails+' emaila projekti</small></section><section><span>Kontakti i fundit</span><b>'+E(since(r.last_seen_at||r.last_contact))+'</b></section></div>'
  +(pending?'<div class="pcm-detail-refreshing">Duke marrë lidhjet më të fundit nga PPPP…</div>':'')
  +'<section class="pcm-detail-section"><h3>Projektet e lidhura</h3>'+projectRows(r)+'</section><section class="pcm-detail-section"><h3>Identitetet / burimet</h3>'+sourceRows(r)+'</section><div class="pcm-detail-actions">'+(r.email?'<a href="mailto:'+E(r.email)+'">Shkruaj email</a>':'')+'<button type="button" data-pcm-close>Mbyll</button></div>';
