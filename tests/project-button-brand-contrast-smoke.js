...
must(/\.pwb3-now>button\{[^}]*border:1px solid #4F97AF[^}]*background:#4F97AF[^}]*color:#fff/.test(wb),'Vazhdo must use #4F97AF with white text');
...
must(/\.ppd-back\{[^}]*border:1px solid #4F97AF[^}]*background:#4F97AF[^}]*color:#fff/.test(desk),'Projects Kthehu must use #4F97AF with white text');
