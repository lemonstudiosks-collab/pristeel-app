...
const livePrivateDispositionHtml=`<script>window.open('/SPIN_PROD/APPLICATION/IPN/DocumentManagement/DocumentForDispositionPrivateFrm.aspx?id=d4bf4e54-737d-471d-892c-75c0c39b23d7&Extension=Doc&fileName=156-Dosja%20e%20Tenderit-Shqip.Docx&DocumentIdMultiple=4255884', '_blank');</script>`;
const privateDispositionUrl=extractKrppIntermediateDownloadUrl(livePrivateDispositionHtml,krppDetail);
assert(privateDispositionUrl.includes('/DocumentForDispositionPrivateFrm.aspx?'),'KRPP protected dossier endpoint must be recognized rather than hidden inside HTML');
assert(privateDispositionUrl.includes('fileName=156-Dosja%20e%20Tenderit-Shqip.Docx'),'KRPP protected disposition URL must preserve the protected filename');
...
const protectedEdge=fs.readFileSync('supabase/functions/pppp-tender-protected-archive-analysis/index.ts','utf8');
...
assert(parserSource.includes('documentfordisposition(?:private)?frm'),'KRPP parser must preserve both public and protected disposition routes');
...
assert(edge.includes('krpp_login_required:')&&edge.includes('krppProtectedDispositionName'),'KRPP private disposition routes must become an explicit login-required state, never a fake download failure');
assert(edge.includes("'X-PPPP-Dossier-Partial'")&&edge.includes("'X-PPPP-Login-Required'"),'Partial KRPP ZIP state must be exposed to the frontend');
assert(edge.includes('Status: ${partial?\'PARTIAL\':\'COMPLETE\'}')&&edge.includes('Requires KRPP login:'),'ZIP index must distinguish partial dossiers and name protected documents');
assert(edge.includes('dossier_complete:access.complete')&&edge.includes('protected_documents:access.protected_documents'),'Analysis response must expose dossier completeness and protected-document evidence');
assert(edge.includes("r.recommendation='REVIEW'")&&edge.includes("r.confidence='low'"),'Incomplete KRPP dossiers must not retain an unsupported GO recommendation');
assert(frontend.includes("X-PPPP-Dossier-Partial")&&frontend.includes("X-PPPP-Login-Required"),'Frontend must read partial/login-required dossier headers');
assert(frontend.includes("data-dossier-complete")&&frontend.includes("DOSJA NUK ËSHTË E PLOTË"),'Frontend must visibly mark an incomplete KRPP dossier');
assert(frontend.includes("p.getAttribute('data-dossier-complete')==='1'"),'Tender dossier readiness must require a complete dossier');
assert(pcw.includes("panel.getAttribute('data-dossier-complete')==='1'"),'Project-Centric Workflow must refuse GO while the dossier is incomplete');
...
assert(protectedEdge.includes("const VERSION='v13'")&&protectedEdge.includes("protected-archive-analysis-v7"),'Protected dossier analysis must expose the same-pass Price Intelligence generation');
