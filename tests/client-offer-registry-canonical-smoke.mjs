...
assert.match(migration, /btrim\(coalesce\(ref, ''\)\) = 'ANF-9203'/);
assert.match(migration, /btrim\(coalesce\(business_ref, ''\)\) = 'ANF-9203'/);
assert.match(migration, /lower\(coalesce\(client, ''\)\) like 'evosys laser%'/);
assert.match(migration, /Expected exactly one EVOSYS ANF-9203 project/);
assert.doesNotMatch(migration, /fc96208d-356c-410a-a356-96ce9e9b4d2f/);
