...
(async()=>{
  ...
  assert.strictEqual(w.document.getElementById('pst-oa-recovery'),null,'Unsaved work must not interrupt login or normal work');
  const savedPage=w.document.createElement('div');savedPage.id='page-workspace-apps';savedPage.className='page active';w.document.body.appendChild(savedPage);
  w.document.getElementById('page-workspace-home').classList.remove('active');
  w.PSTOperatingExperienceV1={currentZone:()=> 'apps'};
  api.apply(false);
  const savedButton=w.document.querySelector('#pst-oa-system-home [data-sys="recovery"]');
  assert(savedButton&&!savedButton.hidden,'System must keep an explicit path to saved work');
  savedButton.click();
  assert(w.document.querySelector('#pst-oa-system-home #pst-oa-recovery'),'Recovery controls open inside System, not as a top banner');
  w.document.querySelector('[data-recover-dismiss]').click();
  assert(w.localStorage.getItem('pristeel_unsaved_1'),'Dismissing the panel must preserve the saved operation');
  assert.strictEqual(writes.length,0,'Inspecting saved work must not replay it automatically');
  dom.window.close();
