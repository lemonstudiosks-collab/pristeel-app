...
assert.match(ui,/function companyGroups\(\)/,'Supplier Master must group contacts by company');
assert.match(ui,/groupEligible\(g,mode\)/,'supplier/manufacturer filters must operate on grouped companies');
assert.match(ui,/data-pcm-person-id/,'each person inside a grouped company must remain individually openable');
assert.match(ui,/filteredGroups\.length\+' kompani/,'procurement list count must report companies rather than duplicate contact rows');
...
assert.match(ui,/<th>Personat e kontaktit<\/th><th>Kompania<\/th><th>Furnizon<\/th><th>Specifikat kryesore<\/th><th>Lokacioni<\/th>/,'Supplier Master must expose one company row with grouped contact people and procurement-first columns');
...
console.log('Contact Master grouped Supplier Master cleanup smoke passed.');
