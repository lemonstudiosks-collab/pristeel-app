...
assert(importer.includes("IMPORT_VERSION='protected-archive-upload-v5'"),'importer v5 must remain canonical');
...
assert(importer.includes('identityVerdict')&&importer.includes('document_tender_mismatch'),'ZIP matching must reject documents explicitly tied to another tender');
...
assert(acceptance.includes("protected-archive-analysis-v6"),'acceptance must require analyzer v6');
assert(acceptance.includes("protected-archive-upload-v5"),'acceptance must require importer v5');
assert(acceptance.includes('sourceBefore.procurement_no'),'production acceptance must preserve source tender identity instead of using a fake procurement reference');
...
console.log('KRPP importer v5 real ZIP production acceptance policy smoke passed.');