...
assert.ok(promotionSql.includes("gmail_message_id")&&promotionSql.includes("lower(coalesce(oc.status, '')) in ('sent', 'replied', 'meeting')"),'TED Project promotion may reuse outreach contacts only when real communication evidence exists');
...
(async()=>{
 ...
 assert.ok(text.includes('Analizo'),'operational focus rows should expose an explicit Albanian analysis action');
 assert.equal(window.document.querySelector('.pst-kek-title').textContent,'Mundësi për PRISTEEL');
 assert.ok(window.document.querySelector('.pst-kek-eye').textContent.includes('PËRSHTATJA'));
 ...
 assert.ok(text.includes('Përshtatja'),'table must expose the capability match in Albanian');
 ...
 assert.ok(tedAwardRow.textContent.includes('Analizo'),'actionable TED winner should have an explicit Albanian analysis action');
