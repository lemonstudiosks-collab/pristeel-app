...
assert.equal(w.document.querySelectorAll('.pst-opp-decision').length,2,'priority engine output should become the visible opportunity cards');
assert.ok(w.document.getElementById('pst-opportunities-all'),'full tender feed must remain available backstage for compatibility');
assert.ok(w.document.getElementById('pst-opportunities-all').contains(w.document.querySelector('.pst-kek-filter')),'existing tender filters must be preserved backstage');
assert.ok(w.document.getElementById('pst-opportunities-all').contains(w.document.querySelector('.pst-kek-card')),'existing tender table must be preserved backstage');
assert.equal(w.document.querySelectorAll('[data-pst-opp-action]').length,0,'direct GO REVIEW NO-GO buttons must not remain on opportunity cards');
assert.equal(w.document.querySelectorAll('[data-pst-opp-tender]').length,2,'whole cards must own the opportunity click');
assert.ok(w.document.querySelector('[data-pst-opp-tender="d1"]').textContent.includes('Hap mundësinë'));
