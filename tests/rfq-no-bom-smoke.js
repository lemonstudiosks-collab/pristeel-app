...
(()=>{
 ...
 const w=dom.window;let legacy=0;
 w.alert=()=>{};w.showPage=()=>{};
 ...
 w.logRfqSent=()=>{};
 w.eval(`let bomRows=[]; let suppliers=[{name:'Aktiva',country:'North Macedonia',lang:'en',cat:'producer',contacts:[{name:'Sales',email:'sales@example.com'}]}]; function generateRfqs(){window.__legacy=(window.__legacy||0)+1;} function goToRfq(){window.__legacyGo=(window.__legacyGo||0)+1;}`);
 w.eval(source);
 ...
 w.eval(`bomRows.push({profile:'HEA',dim:'300'});`);
 w.generateRfqs();
