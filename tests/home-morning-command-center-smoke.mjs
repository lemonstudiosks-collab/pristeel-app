...
assert(source.includes('Veprimet prioritare sot')&&source.includes('Projektet konkrete')&&source.includes('Projekt → Material → Ofertë'),'Morning Home must expose the approved three work zones');
...
assert.equal(home.querySelectorAll('[data-morning-action]').length,1,'Canonical action snapshot must render once');
