...
const nativeNeedle='pristeel-native-ui-v3.js?v=20260901-softperf1';
const nativeAt=roles.indexOf(nativeNeedle);
const projectEmailsAt=roles.indexOf('pristeel-project-emails.js');
must(nativeAt>=0,'roles loader does not reference the fast native UI version');
...
must(roles.includes('s.src=path;'),'stable pre-bootstrap scripts are not cacheable');
must(!roles.includes("path+(path.indexOf('?')>-1?'&':'?')+'pst_live='+String(Date.now())"),'legacy Date.now cache-busting remains in generic pre-bootstrap loader');
must(roles.includes('pristeel-project-emails.js?v='+"'"+'+String(Date.now())')||roles.includes("pristeel-project-emails.js?v='+String(Date.now())"),'protected project-emails cache behavior changed unexpectedly');
...
must(manifest.entrypoints.bootstrapLoaderGitBlobSha==='680b520bfdfa6e5ddbcaaea60d9d28a8b9e037ed','manifest loader SHA does not match fast roles blob');
