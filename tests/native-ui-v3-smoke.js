...
for(const token of ['#806E5A','#7F9299','#F7F5F1','#E4DFD7']) must(core.includes(token),`approved soft palette token missing from core: ${token}`);
...
must(entry.includes('pristeel-native-ui-v3-core.js?v=20260902-softpalette1'),'entry does not load the current native UI core');
...
must(manifest.entrypoints.bootstrapGitBlobSha==='756a65991015be2c06d81352af99c5e9e6f5345a','manifest bootstrap SHA does not match production bootstrap');
