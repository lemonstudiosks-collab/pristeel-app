...
assert.match(window.document.querySelector('[data-opp-back]').textContent,/Kthehu te Prodhuesit ndërkombëtarë/,'Opportunity view must expose a clear back button');
window.document.querySelector('[data-opp-back]').click();
assert.equal(window.document.querySelector('[data-rep-target-view]').hidden,false,'Back button must return to producer/company candidate view');
assert.equal(window.document.querySelector('[data-rep-opportunity-view]').hidden,true,'Back button must hide the project/tender view');
