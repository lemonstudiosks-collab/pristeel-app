...
const gc=classifyAlbaniaSteel({title:'Ndërtim i godinës së re të shkollës',cpvs:['45000000-7'],cpv_text:'45000000-7 - Punë ndërtimi',contract_type:'Punë'});
assert.ok(gc.relevance_score>=55,'general construction GC tender must be retained for steel-package review');
assert.equal(gc.category,'possible');
const specialistGc=classifyAlbaniaSteel({title:'Punime elektrike për shkollën e re',cpvs:['45310000-3'],cpv_text:'45310000-3',contract_type:'Punë'});
assert.ok(specialistGc.relevance_score<55,'specialist non-steel packages must not be promoted as GC steel opportunities');
