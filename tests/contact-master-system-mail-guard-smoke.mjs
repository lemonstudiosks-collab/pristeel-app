...
assert(compact.includes('create or replace function public.pppp_project_contact_system_mail_guard_v1()'),'Project-contact database invariant is missing');
assert(compact.includes('create trigger trg_pppp_project_contact_system_mail_guard_v1'),'Project-contact guard trigger is missing');
...
assert(sql.includes('(^|[+._-])'),'System-mail matching is not token-aware');
assert(sql.includes('([+._-]|$)'),'System-mail token boundary is incomplete');
...
assert(compact.includes('revoke all on function public.pppp_project_contact_system_mail_guard_v1() from public, anon, authenticated'),'New trigger function is exposed to normal API roles');

assert(!/drop\s+(table|schema)|truncate\s+/i.test(sql),'Migration contains destructive broad DDL');

console.log('Contact Master token-aware system-mail guard migration smoke test passed.');
