...
assert(/async function openProject\(/.test(integrity),'Project Workspace must expose one named async entrypoint');
assert(/window\.pstOpenProjectWorkspace=openProject/.test(integrity),'Named project entrypoint must own the public project opener');
assert(/__pstCanonicalOwner='pristeel-project-integrity-ui-v1'/.test(integrity),'Project opener must declare its canonical owner');
