...
(async () => {
  ...
  assert.strictEqual(result[0].type, 'prod', 'Supplier subcontract stayed in NDA');
  assert.strictEqual(result[1].type, 'sale', 'Buyer Werkvertrag stayed in NDA');
  assert.strictEqual(result[2].type, 'nda', 'Real NDA was moved incorrectly');
  assert.ok(writes.some(x => x.path === 'contracts?id=eq.1' && x.body.type === 'prod'), 'Production contract correction was not persisted');
  assert.ok(writes.some(x => x.path === 'contracts?id=eq.2' && x.body.type === 'sale'), 'Sales contract correction was not persisted');
