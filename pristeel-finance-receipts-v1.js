...
var _visionPending={};
var _visionAttemptedAt={};
var VISION_AUTO_COOLDOWN_MS=5*60*1000;
...
async function edgeFetch(name,opts){
  ...
  if(opts.timeoutMs&&typeof AbortController!=='undefined'){
    ...
    timer=setTimeout(function(){try{controller.abort();}catch(e){}},opts.timeoutMs);
  }
  try{
    var request={method:opts.method||'GET',headers:headers,body:opts.body};
    if(controller)request.signal=controller.signal;
    var r=await fetch(_SB_URL+'/functions/v1/'+name+(opts.query||''),request);
    if(r.status===401){
      ...
      if(s&&s.access_token){
        ...
        r=await fetch(_SB_URL+'/functions/v1/'+name+(opts.query||''),request);
      }
    }
    ...
    if(e&&e.name==='AbortError')throw new Error('Leximi vizual zgjati më shumë se 60 sekonda. Provo përsëri.');
    throw e;
  }finally{
...
function statusMeta(s){
  var m={
    ...
    no_text:['OCR lokal pa tekst','#B8461F','#FFF0EB'],
...
function showMessage(text,kind){
  ...
  var ok=kind!=='error';
  el.style.display='';el.style.background=ok?'#EAF5EC':'#FCEDEA';el.style.color=ok?'#2E7D32':'#A33A2E';el.textContent=text;
}
...
function renderSummary(){
  ...
  _rows.forEach(function(r){
    ...
    else if(['needs_ocr','ocr_queued','ocr_processing'].indexOf(r.status)>=0)processing++;
    else if(r.status==='failed'||r.status==='no_text')problem++;
...
async function loadRows(silent){
  ...
  try{
    ...
    if(el)el.innerHTML='<div style="font-size:12px;color:var(--red-text);padding:10px 0">Gabim: '+esc(e.message)+'</div>';
  }
}
...
function startPollingIfNeeded(){
  ...
  _pollTimer=setInterval(function(){
    var v=byId(VIEW_ID);
    if(v&&v.style.display!=='none')loadRows(true);else stopPolling();
  },12000);
}
window.finReceiptShow=function(){
  ...
  setReceiptSurface(true);v.style.display='';clearMessage();loadRows(false);
};
...
async function uploadReceiptFile(f,source,input){
  ...
  try{
    ...
    form.append('source_type',source==='camera'?'camera':'upload');
    ...
    else showMessage('Dokumenti u ruajt. PPPP po e lexon; nëse OCR lokal nuk mjafton, do të provojë automatikisht leximin vizual.','ok');
    if(input)input.value='';
    await loadRows(true);
    var id=res&&res.receipt&&res.receipt.id;
    if(id)setTimeout(function(){window.finReceiptOpen(id);},150);
  }catch(e){
    showMessage('Ngarkimi dështoi: '+e.message,'error');
    if(input)input.value='';
  }
}
...
function cameraShell(){
  ...
  m=document.createElement('div');m.id='fin-receipt-camera-modal-v1';
  m.style.cssText='display:none;position:fixed;inset:0;background:rgba(26,26,25,.58);z-index:1300;padding:22px;overflow:auto';
  m.onclick=function(e){if(e.target===m)window.finReceiptCameraClose();};
...
window.finReceiptCameraCapture=function(){
  ...
  canvas.toBlob(function(blob){
    ...
    var file;try{file=new File([blob],name,{type:'image/jpeg',lastModified:Date.now()});}catch(e){file=blob;}
    window.finReceiptCameraClose();
...
async function loadProjects(){
  ...
  try{_projects=await supaFetch('projects?select=id,name&order=last_activity_at.desc&limit=250')||[];}
  catch(e){try{_projects=await supaFetch('projects?select=id,name&order=created_at.desc&limit=250')||[];}catch(_e){_projects=[];}}
}
...
function modalShell(){
  ...
  m=document.createElement('div');m.id='fin-receipt-modal-v1';
  m.style.cssText='display:none;position:fixed;inset:0;background:rgba(26,26,25,.38);z-index:1200;padding:22px;overflow:auto';
  m.onclick=function(e){if(e.target===m)window.finReceiptClose();};
  document.body.appendChild(m);return m;
}
function replaceRow(r){
  var found=false;
  _rows=_rows.map(function(x){if(String(x.id)===String(r.id)){found=true;return r;}return x;});
  if(!found)_rows.unshift(r);
  renderList();
}
async function fetchReceipt(id){
  var rows=await supaFetch('pppp_expense_receipt_review_v1?select=*&id=eq.'+encodeURIComponent(id)+'&limit=1')||[];
  return rows[0]||null;
}
function visionCanAuto(id){
  var at=Number(_visionAttemptedAt[id]||0);
  return !at||(Date.now()-at)>=VISION_AUTO_COOLDOWN_MS;
}
function visionNote(text,kind){
  var note=byId('fin-receipt-vision-note-v1');if(!note)return;
  note.style.display='';
  note.style.background=kind==='error'?'#FCEDEA':'#EDF5FC';
  note.style.color=kind==='error'?'#A33A2E':'#185FA5';
  note.textContent=text;
}
async function runVisionFallback(id,manual){
  if(!id||_visionPending[id])return false;
  if(!manual&&!visionCanAuto(id))return false;
  _visionPending[id]=true;
  _visionAttemptedAt[id]=Date.now();
  var btn=byId('fin-receipt-vision-btn-v1');
  visionNote('PPPP po e lexon vizualisht faturën. Prit pak; fushat do të plotësohen automatikisht.','info');
  if(btn)btn.disabled=true;
  try{
    var result=await edgeFetch('pppp-expense-receipt-vision',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({receipt_id:id}),
      timeoutMs:60000
    });
    var fresh=await fetchReceipt(id);
    if(fresh){
      replaceRow(fresh);
      if(String(_activeReceiptId)===String(id))await window.finReceiptOpen(id,true);
    }
    return !!(result&&result.ok);
  }catch(e){
    visionNote('Leximi vizual nuk arriti: '+e.message+' Përdor “Lexo përsëri me AI” për një provë tjetër.','error');
    if(btn)btn.disabled=false;
    return false;
  }finally{
    delete _visionPending[id];
  }
}
function startModalPolling(r){
  stopModalPolling();
  if(!r||!_activeReceiptId)return;
  if(['needs_ocr','ocr_queued','ocr_processing'].indexOf(r.status)<0)return;
  _modalPollTimer=setInterval(async function(){
    var id=_activeReceiptId;if(!id){stopModalPolling();return;}
    try{
      var fresh=await fetchReceipt(id);if(!fresh)return;
      replaceRow(fresh);
      if(fresh.status==='no_text'&&String(fresh.mime_type||'').toLowerCase().indexOf('image/')===0){
        stopModalPolling();
        await window.finReceiptOpen(id,true);
        await runVisionFallback(id,false);
        return;
      }
      if(['review','failed','confirmed'].indexOf(fresh.status)>=0){
        stopModalPolling();
        await window.finReceiptOpen(id,true);
      }
    }catch(e){}
...
window.finReceiptVisionRetry=function(){
  if(_activeReceiptId)runVisionFallback(_activeReceiptId,true);
};
window.finReceiptOpen=async function(id,skipAuto){
  var r=_rows.filter(function(x){return String(x.id)===String(id);})[0];
  if(!r){r=await fetchReceipt(id);if(r)replaceRow(r);}
  if(!r){showMessage('Regjistrimi nuk u gjet.','error');return;}
  _activeReceiptId=r.id;
  await loadProjects();
  var signed=await signedPreview(r.id),m=modalShell(),st=statusMeta(r.status),cat=canonicalCategory(r.category),preview='';
  if(signed&&signed.url){
    preview=(String(r.mime_type||'').toLowerCase()==='application/pdf')
      ?'<iframe title="Kuponi PDF" src="'+esc(signed.url)+'" style="width:100%;height:520px;border:1px solid var(--border);border-radius:8px;background:#fff"></iframe>'
      :'<img alt="Kuponi" src="'+esc(signed.url)+'" style="display:block;max-width:100%;max-height:560px;margin:auto;border-radius:8px;border:1px solid var(--border)">';
  ...
  var canVision=editable&&String(r.mime_type||'').toLowerCase().indexOf('image/')===0&&(r.status==='no_text'||r.status==='failed');
  var progress=['needs_ocr','ocr_queued','ocr_processing'].indexOf(r.status)>=0
    ?'<div style="background:#EDF5FC;color:#185FA5;border-radius:7px;padding:8px 10px;font-size:10.5px;margin-bottom:10px">PPPP po lexon faturën. Fushat do të plotësohen automatikisht sapo të përfundojë.</div>'
    :'';
  m.innerHTML='<div style="max-width:1100px;margin:0 auto;background:#fff;border-radius:12px;box-shadow:0 16px 50px rgba(0,0,0,.22);overflow:hidden"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:14px 17px;border-bottom:1px solid var(--border)"><div><div style="font-size:14px;font-weight:700">Kontrolli i kuponit</div><div style="font-size:10.5px;color:var(--text3);margin-top:2px">'+esc(r.file_name)+' · <span style="color:'+st.color+'">'+esc(st.label)+'</span></div></div><button class="btn btn-sm" onclick="finReceiptClose()">✕</button></div>'
    ...
    +(canVision?'<div id="fin-receipt-vision-note-v1" style="background:#EDF5FC;color:#185FA5;border-radius:7px;padding:8px 10px;font-size:10.5px;margin-bottom:10px">OCR lokal nuk mjaftoi. PPPP po kalon në leximin vizual për t’i plotësuar fushat.</div>':'')
    ...
    +(editable
      ?'<button class="btn btn-primary btn-sm" onclick="finReceiptConfirm()">Konfirmo dhe regjistro shpenzimin</button><button class="btn btn-sm" onclick="finReceiptRecalc()">Rillogarit TVSH</button>'+(canVision?'<button id="fin-receipt-vision-btn-v1" class="btn btn-sm" onclick="finReceiptVisionRetry()">Lexo përsëri me AI</button>':'')+'<button class="btn btn-sm" onclick="finReceiptIgnore()">Injoro</button>'
      :'<span style="font-size:11px;color:#2E7D32;font-weight:650">Ky kupon është regjistruar te Shpenzimet operative.</span>')
  ...
  m.style.display='block';
  startModalPolling(r);
  if(!skipAuto&&r.status==='no_text'&&canVision&&visionCanAuto(r.id)){
    await runVisionFallback(r.id,false);
  }
};
window.finReceiptClose=function(){
  var m=byId('fin-receipt-modal-v1');if(m)m.style.display='none';
  stopModalPolling();_activeReceiptId=null;
};
window.finReceiptRecalc=function(){
  var gross=num(byId('fr-amount-v1').value),rate=num(byId('fr-rate-v1').value)||0;
  if(gross==null)return;
  var netv=rate>0?gross/(1+rate/100):gross,vat=gross-netv;
  byId('fr-net-v1').value=netv.toFixed(2);
  byId('fr-vat-v1').value=vat.toFixed(2);
};
window.finReceiptConfirm=async function(){
  ...
  var values={
    supplier:supplier,
    invoice_nr:byId('fr-invoice-v1').value.trim()||null,
    date:date,
    category:canonicalCategory(byId('fr-category-v1').value),
    amount:amount,
    net_amount:num(byId('fr-net-v1').value),
    vat_rate:num(byId('fr-rate-v1').value),
    vat_amount:num(byId('fr-vat-v1').value),
    currency:(byId('fr-currency-v1').value.trim()||'EUR').toUpperCase(),
    project_id:byId('fr-project-v1').value||null,
    vehicle_plate:byId('fr-plate-v1').value.trim()||null,
    quantity:num(byId('fr-qty-v1').value),
    unit:byId('fr-unit-v1').value.trim()||null,
    unit_price:num(byId('fr-unit-price-v1').value),
    notes:byId('fr-notes-v1').value.trim()||null,
    deductible:!!byId('fr-deductible-v1').checked
  };
  try{
    await supaFetch('rpc/pppp_confirm_expense_receipt_v1','POST',{p_receipt_id:_activeReceiptId,p_values:values});
    window.finReceiptClose();
    showMessage('Shpenzimi u regjistrua pas konfirmimit tënd.','ok');
    await loadRows(true);
...
window.finReceiptIgnore=async function(){
  ...
  try{
    await supaFetch('rpc/pppp_ignore_expense_receipt_v1','POST',{p_receipt_id:_activeReceiptId});
    window.finReceiptClose();
    showMessage('Dokumenti u injorua pa krijuar shpenzim.','ok');
    await loadRows(true);
  }catch(e){alert('Veprimi dështoi: '+e.message);}
};
