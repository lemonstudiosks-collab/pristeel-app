...
function questionLike(q){var n=N(q),words=n.split(' ').filter(Boolean);return /^(cka|cfare|kush|ku|kur|pse|si|a ka|a kemi|me trego|trego|cil|what|which|who|where|when|why|how)\b/i.test(n)||/\?/.test(S(q))||words.length<=4;}
...
function latestSchedule(facts){return A(facts).filter(function(f){return N(f.category)==='execution schedule';}).sort(function(a,b){return factTime(b)-factTime(a);})[0]||null;}
...
function executionEvidence(p){
  if(!p||N(p.operational_state)!=='execution')return null;
  var facts=factsForProject(p.id),visit=confirmedVisit(facts),schedule=latestSchedule(facts),sv=schedule&&schedule.value&&typeof schedule.value==='object'?schedule.value:{};
  return{visit:visit,schedule:schedule,summary:S(sv.summary)};
}
function portfolioAnswer(q,hits){
  ...
  var current=rows.find(function(p){return N(p.operational_state)==='execution';}),currentEvidence=executionEvidence(current);
  if(currentEvidence&&currentEvidence.visit)lines.push('Në '+S(current.name||'projektin në ekzekutim')+', vizita në fabrikë është konfirmuar për '+currentEvidence.visit.date+(currentEvidence.visit.time?', rreth orës '+currentEvidence.visit.time:'')+'.');
  if(currentEvidence&&currentEvidence.summary)lines.push(clamp(currentEvidence.summary,650));
  rows.slice(0,10).forEach(function(p){var ref=refLabel(p);lines.push('• '+(ref?ref+' — ':'')+S(p.name||'Projekt')+' — '+stageLabel(p));});
  var nav=current||rows[0],next=currentEvidence&&currentEvidence.visit?'Përgatit vizitën e konfirmuar për '+currentEvidence.visit.date+' dhe zgjidh projektin në ekzekutim për detajet.':'Zgjidh projektin konkret për të parë komunikimet, ofertën dhe hapat e tij.';
  return{ok:true,answer:lines.join('\n'),confidence:'high',uncertainty:'',suggested_next_step:next,navigation:nav?{project_id:nav.id,project_name:nav.name,area:N(nav.operational_state)==='execution'?'execution':'commercial'}:null,evidence:rows.slice(0,10).map(function(p){return{source:'Regjistri kanonik i projekteve',reason:(refLabel(p)?refLabel(p)+' · ':'')+S(p.name)+' · '+stageLabel(p)};}).concat(currentEvidence&&currentEvidence.visit?[{source:'Email i konfirmuar i lidhur me projektin',reason:clamp(rawFactText(currentEvidence.visit.fact),500)}]:[]),provider:{name:'pppp-live-portfolio',model:'deterministic-v1'},read_only:true};
}
...
function installOwnerQueryBridge(owner){
  ...
  try{if(window.PSTUiOwnershipCleanupV1&&typeof window.PSTUiOwnershipCleanupV1.installAskOwnerQueryBridge==='function'){if(window.PSTUiOwnershipCleanupV1.installAskOwnerQueryBridge())return true;}}catch(e){}
  if(owner.__pstAskQueryBridge)return true;
  var original=owner.querySelector.bind(owner);owner.__pstAskOriginalQuerySelector=owner.__pstAskOriginalQuerySelector||original;
  owner.querySelector=function(selector){
    if(selector==='.pst-live-result'||selector==='.pst-live-send'||selector==='.pst-live-input'||selector==='.pst-live-command'){
      var live=document.querySelector('#pst-native-home-v4 .pst-live-command-shell'),found=live&&live.querySelector(selector);if(found)return found;
    }
    return original(selector);
  };
  owner.__pstAskQueryBridge=true;return true;
}
function apply(){
  var home=document.getElementById('pst-native-home-v4'),slot=home&&home.querySelector('#pn-ask'),owner=document.getElementById('pst-project-control-home-v2');
  if(!home||!slot||!owner)return false;
  var shell=null;
  try{var original=owner.__pstAskOriginalQuerySelector;if(typeof original==='function')shell=original('.pst-live-command-shell');if(!shell)shell=Array.prototype.slice.call(owner.children||[]).find(function(x){return x&&x.classList&&x.classList.contains('pst-live-command-shell');})||null;}catch(e){}
  if(!shell)shell=slot.querySelector('.pst-live-command-shell');if(!shell)return false;
  ...
  shell.style.display='block';shell.style.visibility='visible';bindDataLookup(shell);installOwnerQueryBridge(owner);
  ...
  if(observer){observer.disconnect();observer=null;}if(stopTimer){clearTimeout(stopTimer);stopTimer=null;}return true;
}
function watchUntilReady(){
  if(apply())return true;if(observer||!window.MutationObserver)return false;
  var target=document.getElementById('page-workspace-home')||document.body||document.documentElement;if(!target)return false;
  observer=new MutationObserver(function(){apply();});observer.observe(target,{childList:true,subtree:true});
  stopTimer=setTimeout(function(){if(observer){observer.disconnect();observer=null;}stopTimer=null;},120000);return false;
}
...
function loadOpportunityDraftState(){
  ...
  var s=document.createElement('script');s.src='pristeel-opportunity-draft-state-v1.js?v=20260903-draftstate1';s.defer=true;s.setAttribute('data-pst-opportunity-draft-state','1');document.head.appendChild(s);
}
