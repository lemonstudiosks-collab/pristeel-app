...
function urlScore(u,origin,name){
  u=String(u||'');name=String(name||'');if(!documentLike(u,name))return-999;var s=0;
  if(origin==='project-file')s+=70;if(origin==='project')s+=55;if(origin==='buyer-email')s+=35;
  if(/(?:we\.tl|wetransfer|dropbox|sharepoint|onedrive|drive\.google\.com|docs\.google\.com|mega\.|transfer|download)/i.test(u))s+=35;
  if(/\.zip(?:[?#]|$)/i.test(u)||/\.zip$/i.test(name))s+=45;
  if(/\.(?:pdf|dwg|dxf|ifc|xlsx?|docx?|rar|7z)(?:[?#]|$)/i.test(u)||/\.(?:pdf|dwg|dxf|ifc|xlsx?|docx?|rar|7z)$/i.test(name))s+=18;
...
function bestLink(d){
  d=d||data()||{};var out=[],p=d.project||{},drive=preferredDrive(p);if(drive)return{url:drive,origin:'project-drive',name:'',score:999};
  ...
  [p.project_download_url,p.download_url].forEach(function(u){candidate(out,u,'project','');});
...
function inject(){
  ...
  inp.addEventListener('input',function(e){if(e&&e.isTrusted)inp.setAttribute('data-prfq-doc-user-edited','1');open.href=inp.value.trim()||'#';var F=window.PSTRfqDraftFinalizerV1;if(F&&typeof F.finalize==='function')setTimeout(F.finalize,0);});
  return true;
}
...
window.PSTRfqProjectDocumentationV1={inject:inject,patchRows:patchRows,bestLink:bestLink,currentLink:currentLink,_test:{badUrl:badUrl,documentLike:documentLike,urlScore:urlScore}};
})();
