...
var wcScheduled=false,wcBound=false;
...
.pst-wc-hide{display:none!important}\
...
#page-workspace-project .pst-pic:has(.pst-pic-send:disabled) .pst-pic-working-banner{display:flex}\
...
function wcChat(page){Array.prototype.slice.call(page.querySelectorAll('.pst-pic')).forEach(function(root){var banner=root.querySelector('.pst-pic-working-banner');if(!banner){banner=document.createElement('div');banner.className='pst-pic-working-banner';banner.innerHTML='<i class="pst-pic-working-dot"></i><div><b>PPPP po punon me projektin…</b><span>Po lexoj të dhënat, emailat dhe dokumentet. Mund të zgjasë disa sekonda.</span></div>';var log=root.querySelector('.pst-pic-log');if(log&&log.nextSibling)root.insertBefore(banner,log.nextSibling);else root.appendChild(banner);}});}
function wcRun(){wcScheduled=false;wcCss();var page=wcPage();if(!page){wcUnhide();return;}wcHideTop(page);wcBack(page);wcHero(page);wcCanonical(page);wcRemoveDeadUpdate(page);wcChat(page);}
function wcSchedule(){if(wcScheduled)return;wcScheduled=true;setTimeout(wcRun,0);}
function wcBurst(){[0,80,250,700,1600,4000].forEach(function(ms){setTimeout(wcSchedule,ms);});}
function installWorkspaceCleanup(){wcCss();if(!wcBound){wcBound=true;document.addEventListener('click',function(e){var t=e.target&&e.target.closest&&e.target.closest('#page-workspace-project,[data-pm-open],[data-project-id],.pst-pm-open');if(t)wcBurst();},true);document.addEventListener('pst:modules-ready',wcBurst);}wcBurst();return true;}
