...
function buyerBody(r){
 var mat=materialText(r),indicative=r.quote_readiness!=='M3',project=S(r.project_title||'');
 return 'Guten Tag,\n\nim Zusammenhang mit dem Projekt „'+project+'“ möchten wir anfragen, ob die Materialbeschaffung für den relevanten Stahlbauumfang noch offen ist.\n\nPRISTEEL kann projektbezogene Stahlmaterialien aus unserem Beschaffungsnetzwerk liefern.\n'+(indicative?'Auf Grundlage der veröffentlichten Projektunterlagen haben wir derzeit folgenden voraussichtlichen Stahlumfang identifiziert:':'Auf Grundlage der verfügbaren Projektunterlagen haben wir folgenden Stahlumfang identifiziert:')+'\n\n'+mat+'\n\n'+(indicative?'Hinweis: Die oben genannten Mengen sind vorläufig und dienen nur zur Einordnung.':'Die Positionen entsprechen der derzeit verfügbaren Materialbasis.')+'\n\nFalls die Beschaffung noch nicht abgeschlossen ist, senden Sie uns bitte die aktuelle Materialliste / BOQ sowie – soweit verfügbar – Zeichnungen und Spezifikationen. Auf dieser Grundlage erstellen wir kurzfristig ein technisches und kommerzielles Angebot.\n\nSollte hierfür eine andere Person im Einkauf oder in der Projektleitung zuständig sein, wäre ich für eine Weiterleitung dankbar.\n\nMit freundlichen Grüßen\nArianit Vllahiu\nHead of Business Development\n+383 (0) 44 244 699\narianit.vllahiu@prissteel.com\nwww.prissteel.com';
}
...
function ensurePage(){
 ...
 page.onclick=async function(e){
  ...
  if(btn){
   ...
   if(act==='buyer-find-contact'){await loadBuyerContact(r,true);return}
  ...
  var row=e.target.closest('[data-dss-target-id]');if(row){var rid=row.getAttribute('data-dss-target-id'),openIt=state.expanded!==rid;state.expanded=openIt?rid:null;state.actionView=null;renderPage();if(openIt){var rr=findTarget(rid);if(rr)loadBuyerContact(rr,false)}}
 };
 return page;
}
...
function buyerAction(r){
 ...
 var preview=state.actionView&&state.actionView.id===S(r.id)&&state.actionView.type==='buyer',k=draftKey('buyer',r.id),busy=!!state.draftBusy[k],result=state.draftResult[k]||null,primary='',secondary='',cbusy=!!state.contactBusy[S(r.id)];
 if(life==='project')primary='<button class="pst-dss-btn success" disabled>✓ Projekt aktiv në PPPP</button>';
 else if(life==='replied')primary=q&&q.gmail_thread_id?'<button class="pst-dss-btn primary" data-dss-action="buyer-thread" data-dss-tid="'+E(r.id)+'">Hap përgjigjen në Gmail</button>':'';
 ...
 else primary='<button class="pst-dss-btn" '+(cbusy?'disabled':'')+' data-dss-action="buyer-find-contact" data-dss-tid="'+E(r.id)+'">'+(cbusy?'Duke kërkuar…':'Kërko kontakt')+'</button>';
 if(life==='action')secondary='<button class="pst-dss-btn" data-dss-action="buyer-preview" data-dss-tid="'+E(r.id)+'">Shiko tekstin</button>';
 var contactLine=to?'<b>'+E(to)+'</b>'+(ct.person?' · '+E(ct.person):'')+(ct.role?' · '+E(ct.role):'')+(ct.source?' · '+E(ct.source):''):'Kontakt ende i pazgjidhur';
 var guard=life==='project'?'Ky target është promovuar në Projektet dhe vazhdon në workflow-n canonical të projektit.':
   life==='replied'?('Përgjigje e marrë'+(replyAt?' më '+D(replyAt):'')+'. Nëse përmban RFQ/BOQ real, hapi tjetër është promovimi në Projektet me aprovim human.'):
   life==='waiting'?('Emaili është dërguar'+(sentAt?' më '+D(sentAt):'')+'. Targeti qëndron te “Në ndjekje”; outreach i ri bllokohet nga cooldown-i.'):
   ...
   to?'Kontakti është zgjidhur. Para draftit PPPP kontrollon Gmail Sent + shared cooldown; dërgimi mbetet human-approved.':
   'PPPP po përdor Contact Master + evidence. Nëse nuk gjendet kontakt i sigurt, drafti nuk krijohet.';
 var resultHtml=result&&result.error?'<div class="pst-dss-inline-status" style="background:#fff1ef;color:#8b4a41">Drafti nuk u krijua: '+E(result.error)+'</div>':(result&&result.created?'<div class="pst-dss-inline-status">✓ Gmail draft u krijua për '+E(result.recipient||to)+' dhe u lidh me PPPP.</div>':'');
 return '<div class="pst-dss-action-card"><h4>📩 Blerësi · '+(life==='project'?'projekt aktiv':life==='replied'?'përgjigje e marrë':life==='waiting'?'në pritje të RFQ / BOQ':'kërko RFQ / BOQ')+'</h4><p>'+(life==='project'?'Ky rast nuk është më lead i ftohtë; ndiqet si projekt canonical.':life==='action'?'Kontakti më i mirë zgjidhet automatikisht. Pastaj kërko material listën aktuale, drawings/specs dhe konfirmo nëse procurement-i është ende i hapur.':life==='waiting'?'Emaili u dërgua. Tani monitorojmë reply/RFQ pa e kontaktuar sërish gjatë cooldown-it.':'Ka ardhur përgjigje. Verifiko nëse kemi RFQ, BOQ, drawings ose kërkesë konkrete.')+'</p><div class="pst-dss-action-status">'+contactLine+'</div><div class="pst-dss-action-buttons">'+primary+secondary+'</div><div class="pst-dss-guard">'+E(guard)+'</div>'+resultHtml+(preview?'<div class="pst-dss-previewbox"><b>'+E(buyerSubject(r,q))+'</b><pre>'+E(buyerBody(r))+'</pre><div class="pst-dss-action-buttons" style="margin-top:9px"><button class="pst-dss-btn" data-dss-action="buyer-copy" data-dss-tid="'+E(r.id)+'">Kopjo tekstin</button></div></div>':'')+'</div>';
}
function supplierAction(r){
 ...
 if(box.loading)candHtml='<div class="pst-dss-previewbox"><b>Duke kontrolluar Supplier Intelligence…</b></div>';
  ...
  candHtml='<div class="pst-dss-candidates">'+(candidates.length?candidates.map(function(c){
   var fit=c.strict_fit?'Strict fit':'Review fit',cl=c.strict_fit?'pst-dss-fit':'pst-dss-fit review',email=S(c.email||''),dk=draftKey('supplier',r.id,email),busy=!!state.draftBusy[dk],dr=state.supplierDrafts[dk]||null,action='';
   ...
   return '<div class="pst-dss-candidate"><div><b>'+E(c.name||'Supplier')+'</b><small>'+E([c.country,email,'score '+S(c.match_score||0)].filter(Boolean).join(' · '))+(dr&&dr.error?'<br><span style="color:#9a4d43">Draft error: '+E(dr.error)+'</span>':'')+'</small></div><span class="'+cl+'">'+E(fit)+'</span>'+action+'</div>';
