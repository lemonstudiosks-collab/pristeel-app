...
'#page-dach-steel-sales .pst-dss-primary-wide{width:100%;margin-top:14px;border:0;border-radius:10px;background:#176fa8;color:#fff!important;-webkit-text-fill-color:#fff!important;padding:11px 12px;font-size:11px;font-weight:850;cursor:pointer}#page-dach-steel-sales .pst-dss-primary-wide:hover,#page-dach-steel-sales .pst-dss-primary-wide:focus,#page-dach-steel-sales .pst-dss-primary-wide:active{background:#125f92;color:#fff!important;-webkit-text-fill-color:#fff!important}#page-dach-steel-sales .pst-dss-primary-wide:disabled{opacity:1;cursor:not-allowed;color:#fff!important;-webkit-text-fill-color:#fff!important}.pst-dss-secondary-wide{width:100%;margin-top:8px;border:1px solid #cfe0ea;border-radius:10px;background:#fff;color:#2d6f93;padding:9px 11px;font-size:10.5px;font-weight:800;cursor:pointer}',
...
function hasOutboundHistory(r){
 var q=outboundFor(r);if(!q)return false;
 var st=N(q.status),supp=N(q.suppression_reason);
 return !!(q.gmail_draft_id||q.gmail_thread_id||q.sent_at||q.replied_at||st==='sent'||st==='replied'||st==='stale'||['gmail_draft_missing','gmail_draft_stale','draft_missing','draft_stale'].indexOf(supp)>-1);
}
function contactedLifecycle(r){
 ...
 if(st==='stale'||['gmail_draft_missing','gmail_draft_stale','draft_missing','draft_stale'].indexOf(supp)>-1)return'stale';
...
function evidenceContact(r){
 ...
 ev.forEach(function(x){
  var obj=(x&&typeof x==='object')?x:{label:S(x)},raw=[obj.email,obj.label,obj.claim,obj.title,obj.source].filter(Boolean).join(' '),email=emailFrom(raw);if(!email)return;
  var d=N(email.split('@')[1]||''),label=N(raw),general=/^(info|kontakt|office|mail|zentrale)@/i.test(email);
  var cand={email:email,person:S(obj.person||''),role:/(einkauf|purchas|procurement|service achats|ufficio acquisti|nabavk|achats|compras|zakupy)/.test(label)?'Purchasing / Procurement':label.indexOf('commercial department')>-1?'Commercial department':label.indexOf('official company contact')>-1?'Official company contact':label.indexOf('company contact')>-1?'Public company contact':'Public contact',source:'target_evidence',quality:general?'general':'direct_public',source_url:S(obj.url||r.source_url||'')};
...
function supplierAction(r){
 var indicative=r.quote_readiness!=='M3',box=state.supplierByTarget[S(r.id)]||{},preview=state.actionView&&state.actionView.id===S(r.id)&&state.actionView.type==='supplier',rfqUnlocked=!!S(r.project_id)&&lifecycle(r)==='replied';
 ...
 if(box.loading)candHtml='<div class="pst-dss-previewbox"><b>Duke kontrolluar inteligjencën e furnitorëve…</b></div>';
  ...
  candHtml='<div class="pst-dss-candidates">'+(candidates.length?candidates.map(function(c){
   ...
   else if(email&&rfqUnlocked)action='<button class="pst-dss-btn" '+(busy?'disabled':'')+' data-dss-action="supplier-create-draft" data-dss-tid="'+E(r.id)+'" data-email="'+E(email)+'">'+(busy?'Duke kontrolluar…':'Krijo RFQ draft')+'</button>';
   else if(email)action='<span class="pst-dss-fit review">RFQ pas reply → Project</span>';
 ...
 return '<div class="pst-dss-action-card"><h4>🏭 Furnizimi · '+E(label)+'</h4><p>'+(indicative?'M2/M1: kërkesë indikative për furnizim. Sasitë dhe specifikat duhet të konfirmohen para ofertës finale.':'M3: kemi bazë BOQ / listë materiali dhe mund të përgatitet RFQ e plotë.')+'</p><div class="pst-dss-action-status"><b>'+E(tonnes(r.estimated_tonnes))+'</b> · '+E(qrLabel(r.quote_readiness))+'</div><div class="pst-dss-action-buttons"><button class="pst-dss-btn primary" data-dss-action="find-suppliers" data-dss-tid="'+E(r.id)+'">'+(box.loaded?'Rifresko furnitorët':'Gjej furnitorë')+'</button><button class="pst-dss-btn" data-dss-action="supplier-preview" data-dss-tid="'+E(r.id)+'">Shiko RFQ</button></div><div class="pst-dss-guard">'+(rfqUnlocked?'Buyer reply + Project janë konfirmuar. Supplier RFQ kalon Supplier Gate para se të hapet Gmail.':'Këtu bëhet vetëm discovery/read-only i furnitorëve. Supplier RFQ mbetet i bllokuar derisa të kemi reply/RFQ real nga blerësi dhe targeti të promovohet në Project.')+'</div>'+(preview?'<div class="pst-dss-previewbox"><b>'+E(supplierSubject(r,indicative))+'</b><pre>'+E(supplierBody(r,{}))+'</pre></div>':'')+candHtml+'</div>';
}
function detail(r){
 var ev=A(J(r.evidence,[])),scope=J(r.material_scope,{}),conf=r.material_confidence!=null?Math.round(num(r.material_confidence)*100)+'%':'—',ct=contactFor(r),email=S(ct.email||''),site=U(r.company_website||r.source_url),source=U(r.source_url);
 var refs=ev.length?'<div class="pst-dss-evidence">'+ev.slice(0,10).map(function(x){
  var o=(x&&typeof x==='object')?x:{label:S(x)},label=o.label||o.title||o.source||o.url||'Burim',claim=S(o.claim||''),url=U(o.url||'');
  return '<div style="padding:9px 10px;border:1px solid #e5e9e7;border-radius:8px;background:#fbfcfb"><b style="font-size:10.5px;color:#4b636c">'+E(label)+'</b>'+(claim?'<div style="margin-top:4px;font-size:10.5px;line-height:1.45;color:#687a81">'+E(claim)+'</div>':'')+(url?'<a class="pst-dss-source" style="margin-top:5px" href="'+E(url)+'" target="_blank" rel="noopener">Hap burimin ↗</a>':'')+'</div>';
 }).join('')+'</div>':'<div class="pst-dss-empty" style="padding:12px"><b>Nuk ka evidencë publike të ruajtur</b></div>';
 var companyContext=r.project_title
  ?'<span>Projekt/ref. publike</span><span>'+E(r.project_title)+'</span>'
  :'<span>Konteksti</span><span>Target tregtar i kompanisë; nuk varet nga një tender apo projekt specifik.</span>';
 var contactBlock=email
  ?'<div class="pst-dss-contact"><b><a href="mailto:'+E(email)+'">'+E(email)+'</a></b><small>'+E([ct.person,ct.role].filter(Boolean).join(' · ')||ct.quality||'Kontakt publik')+'</small></div>'
  :'<span class="pst-dss-contact-missing">Nuk kemi ende email të verifikuar</span>';
 return '<div class="pst-dss-detail">'
  +'<div class="pst-dss-summary">'
   +'<section class="pst-dss-summary-card"><h3>Kompania</h3><div class="pst-dss-summary-title">'+E(r.company_name||'Blerës')+'</div><div class="pst-dss-summary-sub">'+E(r.why_now||'Arsyeja e kualifikimit ende mungon.')+'</div><div class="pst-dss-keyfacts"><span>'+E(r.country||'—')+'</span><span>'+E(buyerTierLabel(r))+'</span><span>'+E(r.score_band||'—')+'</span><span>Besueshmëria '+E(conf)+'</span></div>'+(site?'<a class="pst-dss-source" href="'+E(site)+'" target="_blank" rel="noopener">Website i kompanisë ↗</a>':'')+'</section>'
   +'<section class="pst-dss-summary-card"><h3>Kontakti për blerje</h3>'+contactBlock+'<div class="pst-dss-summary-sub" style="margin-top:9px">'+E(r.next_action||'Verifiko kontaktin dhe përgatit outreach vetëm pas rishikimit.')+'</div></section>'
  +'</div>'
  +'<div class="pst-dss-summary">'
   +'<section class="pst-dss-summary-card"><h3>Çfarë prodhon / konsumon</h3><div class="pst-dss-summary-title">'+E(arrText(r.products).join(' · ')||'Steel material')+'</div><div class="pst-dss-summary-sub">'+E(r.steel_scope||'Fusha e materialit ende nuk është përmbledhur.')+'</div></section>'
   +'<section class="pst-dss-summary-card"><h3>Konteksti tregtar</h3><div class="pst-dss-meta"><span>Lloji i blerësit</span><span>'+E(r.buyer_type||'—')+'</span>'+companyContext+'<span>Website/domain</span><span>'+E(r.company_domain||'—')+'</span><span>Verifikuar</span><span>'+E(D(r.last_verified_at))+'</span></div></section>'
  +'</div>'
  +'<section class="pst-dss-action-center"><header><b>Çfarë bëjmë tani?</b><span>Blerësi + furnizimi</span></header><div class="pst-dss-action-grid">'+buyerAction(r)+supplierAction(r)+'</div></section>'
  +'<details class="pst-dss-details" open><summary>Evidenca publike për kompaninë</summary><div class="pst-dss-detail-grid"><section class="pst-dss-detail-card"><h3>Pse është target</h3><div class="pst-dss-summary-sub">'+E(r.why_now||'—')+'</div>'+(source?'<a class="pst-dss-source" href="'+E(source)+'" target="_blank" rel="noopener">Burimi kryesor ↗</a>':'')+'</section><section class="pst-dss-detail-card"><h3>Burime dhe kontakte</h3>'+refs+'</section></div></details>'
 +'</div>';
}
