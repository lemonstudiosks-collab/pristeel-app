 ...
 * Revision: 20260823-quality-readiness1.
 * Re-applies presentation styling and loads read-only presentation children after workspace renders.
...
function contactCards(){
  try{
    ...
    var s=document.createElement('script');
    s.src='pristeel-contact-category-cards-v1.js?v=20260822-1';
    s.defer=true;
    s.setAttribute('data-pst-contact-category-cards','1');
    s.onload=function(){var x=window.PSTContactCategoryCardsV1;if(x&&typeof x.decorate==='function')x.decorate();};
    document.head.appendChild(s);
  }catch(e){console.warn('PRISTEEL finalizer contact cards:',e);}
}
function qualityReadiness(){
  try{
    var Q=window.PSTQualityReadinessV1;
    if(Q&&typeof Q.load==='function'){Q.load(false);return;}
    if(document.querySelector('script[data-pst-quality-readiness]'))return;
    var s=document.createElement('script');
    s.src='pristeel-quality-readiness-v1.js?v=20260823-1';
    s.defer=true;
    s.setAttribute('data-pst-quality-readiness','1');
    document.head.appendChild(s);
  }catch(e){console.warn('PRISTEEL finalizer quality readiness:',e);}
}
function priorityIsUrgent(row){
  if(!row)return false;
  var title=row.dataset&&row.dataset.pstOriginalTitle||'';
  if(!title){var t=row.querySelector('.pst-ws-action-title');title=t?String(t.getAttribute('title')||t.textContent||''):'';}
  var tag=row.querySelector('.pst-ws-action-tag'),label=tag?String(tag.textContent||''):'';
...
function repairPriorityControls(row){
  if(!row)return;var controls=row.querySelector('.pst-ws-action-controls');if(!controls)return;
  var canonicalOpen=row.querySelector('.pst-ws-action-open'),canonicalDone=row.querySelector('.pst-ws-action-done'),canonicalDismiss=row.querySelector('.pst-ws-action-dismiss'),source=row.querySelector('.pst-task-source-open');
  row.querySelectorAll('.pst-dash-task-open').forEach(function(b){if(b.parentNode)b.remove();});
  if(canonicalDone){canonicalDone.textContent='Kryer';canonicalDone.title='Shënoje si të kryer';canonicalDone.classList.remove('pst-dash-task-dismiss');}
  if(canonicalDismiss){canonicalDismiss.textContent='•••';canonicalDismiss.title='Hiqe nga lista';}
  if(canonicalOpen)controls.appendChild(canonicalOpen);if(canonicalDone)controls.appendChild(canonicalDone);if(source)controls.appendChild(source);if(canonicalDismiss)controls.appendChild(canonicalDismiss);
  row.querySelectorAll('.pst-dash-task-menu').forEach(function(menu){if(menu.parentNode)menu.remove();});
}
function repairPriorityCards(){
  var page=document.getElementById('page-workspace-home');if(!page)return 0;var count=0;
  page.querySelectorAll('#pst-ws-home-actions > .pst-canonical-action').forEach(function(row){repairPriorityControls(row);row.classList.toggle('pst-final-priority-urgent',priorityIsUrgent(row));row.style.cursor='pointer';row.setAttribute('title','Kliko për ta hapur');count++;});
  return count;
}
function installPriorityStyle(){
  if(document.getElementById('pst-final-priority-card-css'))return;
  var s=document.createElement('style');s.id='pst-final-priority-card-css';s.textContent=`
...
function apply(){
  sectionTheme();readability();contactCards();qualityReadiness();
...
document.addEventListener('click',function(event){var t=event.target&&event.target.closest?event.target.closest('.pst-ws-navbtn,#pst-ws-home-refresh,[onclick*="pstWorkspaceGo"],[data-pm-open],[data-release-filter],[onclick*="showPage"],[onclick*="openModuleHub"],[data-pwf-area]'):null;if(t)[0,80,250,700].forEach(function(ms){setTimeout(apply,ms);});},true);
if(document.readyState!=='loading')schedule();
window.PSTRedesignFinalizerV1={apply:apply,schedule:schedule,readability:readability,sectionTheme:sectionTheme,contactCards:contactCards,qualityReadiness:qualityReadiness,repairPriorityCards:repairPriorityCards};
})();
