...
function dailyZones(){
  try{
    var X=window.PSTDailyZonesCleanupV1;
    if(X&&typeof X.apply==='function'){X.apply();return;}
    if(document.querySelector('script[data-pst-daily-zones-cleanup]'))return;
    var s=document.createElement('script');
    s.src='pristeel-daily-zones-cleanup-v1.js?v=20260825-1';
    s.defer=true;
    s.setAttribute('data-pst-daily-zones-cleanup','1');
    s.onload=function(){var x=window.PSTDailyZonesCleanupV1;if(x&&typeof x.apply==='function')x.apply();};
    document.head.appendChild(s);
  }catch(e){console.warn('PRISTEEL finalizer daily zones:',e);}
}
