...
box.querySelectorAll('[data-pdc-open]').forEach(function(b){b.onclick=function(){var target=b.getAttribute('data-pdc-open');if(typeof window.pstOpenProjectWorkspace==='function')window.pstOpenProjectWorkspace(target);};});return true;}
function wrap(){var base=window.pstOpenProjectWorkspace;if(typeof base!=='function'||base.__pdc)return false;async function f(id){var r=await base.apply(this,arguments);setTimeout(function(){inspect(id);},40);return r;}f.__pdc=true;f.__base=base;window.pstOpenProjectWorkspace=f;return true;}
function install(){if(!wrap())setTimeout(wrap,250);var id=window.__pstCurrentProjectId||window._curProjId;if(id)setTimeout(function(){inspect(id);},350);}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
document.addEventListener('pst:modules-ready',function(){setTimeout(install,50);},{once:true});
