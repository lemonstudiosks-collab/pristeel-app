/* PRISTEEL — Tender Monitor v2
 * Review-first intake for steel-relevant KEK/KRPP and TED notices.
...
function source(row){return String(payload(row).source||'KEK').toUpperCase()==='TED'?'TED':'KEK';}
function phase(row){return payload(row).notice_phase==='award'?'award':'opportunity';}
function sourceLabel(row){return source(row)==='TED'?'TED':'KEK';}
function codeLabel(row){return source(row)==='TED'?'CPV':'FPP';}
...
function addAppsTile(){
 var grid=document.querySelector('#page-workspace-apps .pst-ws-appgrid');if(!grid)return;var old=grid.querySelector('[data-pst-kek-app]');if(old){var name=old.querySelector('.pst-ws-app-name');var sub=old.querySelector('.pst-ws-app-sub');if(name)name.textContent='Tenderat';if(sub)sub.textContent='Monitorimi automatik KEK + TED për lëndë të parë dhe struktura çeliku';return;}
 var b=document.createElement('button');b.className='pst-ws-app';b.setAttribute('data-pst-kek-app','1');b.onclick=function(){window.pstWsKekTenders();};b.innerHTML='<div class="pst-ws-app-icon">'+icon()+'</div><div class="pst-ws-app-name">Tenderat</div><div class="pst-ws-app-sub">Monitorimi automatik KEK + TED për lëndë të parë dhe struktura çeliku</div>';grid.insertBefore(b,grid.firstChild);
}
function installLegacyRegistration(){
 try{
  if(typeof window.pageMeta==='object')window.pageMeta['kek-tenders']={title:'Tenderat',sub:'Monitorimi automatik KEK + TED'};
...
window.pstKekRender=function(){
 ...
 h.innerHTML='<table class="pst-kek-table"><thead><tr><th>Tenderi</th><th>Burimi</th><th>Kategoria</th><th>Relevanca</th><th>Publikuar</th><th>Afati</th><th>Statusi</th><th></th></tr></thead><tbody>'+list.map(function(r){
  var reasons=Array.isArray(r.match_reasons)?r.match_reasons.join(' · '):'';var isAward=phase(r)==='award';var srcName=sourceLabel(r);var sourceBtn=srcName==='TED'?'TED ↗':'KRPP ↗';var meta=esc(r.procurement_no)+(r.fpp?' · '+codeLabel(r)+' '+esc(r.fpp):'')+(r.authority?' · '+esc(r.authority):'');
  var actions=(url(r)?'<button class="pst-kek-btn" onclick="pstKekOpenSource(\''+esc(r.id)+'\')">'+sourceBtn+'</button>':'');
  ...
  return '<tr><td><div class="pst-kek-name">'+esc(r.title)+'</div><div class="pst-kek-meta">'+meta+'</div></td><td><span class="pst-kek-chip source '+(srcName==='TED'?'ted':'')+'">'+srcName+'</span>'+(isAward?'<span class="pst-kek-chip award">Rezultat</span>':'')+'</td><td><span class="pst-kek-chip '+esc(r.category)+'">'+esc(catLabel(r.category))+'</span></td><td><div class="pst-kek-score">'+esc(r.relevance_score)+'%</div><div class="pst-kek-reason">'+esc(reasons)+'</div></td><td>'+dateText(r.published_date)+'</td><td>'+dateText(r.deadline)+'</td><td>'+esc(statusLabel(r.status))+'</td><td><div class="pst-kek-rowacts">'+actions+'</div></td></tr>';
 }).join('')+'</tbody></table>';
};
...
window.pstKekPromote=async function(id){
 ...
 try{
  var existing=await db('projects?ref=eq.'+encodeURIComponent(r.procurement_no)+'&select=id,name,ref&limit=2');var project=Array.isArray(existing)&&existing[0]?existing[0]:null;var src=source(r);
  if(project){if(!confirm('Ky numër prokurimi ekziston tashmë si projekt: '+(project.name||r.procurement_no)+'.\n\nTa lidhim tenderin me projektin ekzistues?'))return;}
   ...
   if(!confirm('Të krijohet projekt i ri nga ky tender '+src+'?\n\n'+r.title+'\n'+r.procurement_no))return;
   var p=payload(r);await db('projects','POST',{name:r.title,client:src==='KEK'?'KEK':(r.authority||'TED'),ref:r.procurement_no,location:src==='KEK'?'Kosovë':(p.country||''),deadline:r.deadline||'',status:'pritje',pipeline_stage:'rfq_in',notes:'Burimi: '+src+'\nNumri i publikimit: '+(r.publication_no||'—')+'\n'+codeLabel(r)+': '+(r.fpp||'—')+'\nKategoria automatike: '+catLabel(r.category)+' ('+r.relevance_score+'%)\nURL: '+(url(r)||'—')});
