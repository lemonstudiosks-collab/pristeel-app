...
.pst-rep-pipeline{display:flex;gap:6px;overflow:auto;padding:3px 0 12px}.pst-rep-pipe{border:1px solid var(--rep-line);background:#fff;border-radius:999px;padding:7px 10px;white-space:nowrap;font-size:9.5px;color:#61746e;cursor:pointer}.pst-rep-pipe.on{background:var(--rep-soft);border-color:#acc6bb;color:var(--rep-dark);font-weight:850}.pst-rep-pipe b{margin-left:5px}
.pst-rep-toolbar{display:grid;grid-template-columns:minmax(220px,1.4fr) repeat(4,minmax(130px,.7fr));gap:8px;margin-bottom:12px}.pst-rep-toolbar input,.pst-rep-toolbar select{height:38px;border:1px solid var(--rep-line);border-radius:10px;background:#fff;padding:0 11px;font-size:10.5px;color:#425952}
...
.pst-rep-home{width:100%;border:1px solid #dbe7e2;background:linear-gradient(135deg,#f6faf8,#fff);border-radius:14px;padding:17px;text-align:left;display:grid;grid-template-columns:1fr auto;gap:18px;align-items:center;cursor:pointer;margin-bottom:13px}.pst-rep-home-eye{font-size:8.5px;letter-spacing:.11em;text-transform:uppercase;font-weight:850;color:#668177}.pst-rep-home-title{font-size:17px;font-weight:830;color:#294b40;margin-top:5px}.pst-rep-home-sub{font-size:10px;color:#748780;line-height:1.5;margin-top:4px}.pst-rep-home-stats{display:grid;grid-template-columns:repeat(4,70px);gap:6px}.pst-rep-home-stat{background:#fff;border:1px solid #e0e9e5;border-radius:10px;padding:8px;text-align:center}.pst-rep-home-stat b{display:block;font-size:17px;color:#345e50}.pst-rep-home-stat span{font-size:7.8px;color:#83928d}
@media(max-width:1100px){.pst-rep-shell{grid-template-columns:1fr}.pst-rep-detail{position:static}.pst-rep-table-head,.pst-rep-row{grid-template-columns:minmax(180px,1.3fr) 80px 110px 105px 60px minmax(130px,.9fr)}.pst-rep-table-head>*:last-child,.pst-rep-row>*:last-child{display:none}.pst-rep-toolbar{grid-template-columns:1fr 1fr 1fr}.pst-rep-toolbar>*:first-child{grid-column:1/-1}}
@media(max-width:760px){.pst-rep-page{padding:12px 0 38px}.pst-rep-head{padding:0 12px;display:block}.pst-rep-head h1{font-size:24px}.pst-rep-actions{margin-top:12px}.pst-rep-kpis{grid-template-columns:1fr 1fr;padding:0 10px}.pst-rep-pipeline{padding:2px 10px 11px}.pst-rep-toolbar{grid-template-columns:1fr 1fr;padding:0 10px}.pst-rep-toolbar>*:first-child{grid-column:1/-1}.pst-rep-toolbar>*:last-child{grid-column:1/-1}.pst-rep-card{border-radius:0;border-left:0;border-right:0}.pst-rep-table-head{display:none}.pst-rep-row{grid-template-columns:1fr auto;padding:13px}.pst-rep-row>*:nth-child(n+3){display:none}.pst-rep-grid{grid-template-columns:1fr}.pst-rep-field.two,.pst-rep-field.full{grid-column:auto}.pst-rep-modal{padding:0}.pst-rep-dialog{height:100vh;max-height:100vh;border-radius:0}.pst-rep-home{grid-template-columns:1fr}.pst-rep-home-stats{grid-template-columns:repeat(4,1fr)}}
...
function ensurePage(){
 ...
 [['stage','[data-rep-stage]'],['country','[data-rep-country]'],['sector','[data-rep-sector]'],['capital','[data-rep-capital]'],['sort','[data-rep-sort]']].forEach(function(x){p.querySelector(x[1]).onchange=function(){state[x[0]]=this.value;render()}});
 ...
 p.querySelector('[data-rep-pipeline]').addEventListener('click',function(e){var b=e.target.closest('[data-rep-pipe]');if(!b)return;state.stage=b.dataset.repPipe;render()});
...
function render(){
 ...
 p.querySelector('[data-rep-pipeline]').innerHTML='<button class="pst-rep-pipe '+(!state.stage?'on':'')+'" data-rep-pipe="">Të gjitha <b>'+act.length+'</b></button>'+STAGES.map(function(x){return'<button class="pst-rep-pipe '+(state.stage===x[0]?'on':'')+'" data-rep-pipe="'+x[0]+'">'+E(x[1])+' <b>'+(counts[x[0]]||0)+'</b></button>'}).join('');
 p.querySelector('[data-rep-stage]').innerHTML=opts(STAGES,state.stage,'Të gjitha stages');
...
function ensureHome(){
 var home=document.getElementById('pst-home-launchpad-v1'),grid=home&&home.querySelector('.pst-launch-grid');if(!home||!grid)return false;
 var card=document.getElementById('pst-representations-home-v1');if(!card){card=document.createElement('section');card.id='pst-representations-home-v1';grid.parentNode.insertBefore(card,grid);card.onclick=open}
...
function renderHome(){
 ...
 card.innerHTML='<button class="pst-rep-home" type="button"><div><div class="pst-rep-home-eye">PËRFAQËSIME · PRODHUES NDËRKOMBËTARË</div><div class="pst-rep-home-title">Zhvillim tregu pa barrë të madhe kapitali</div><div class="pst-rep-home-sub">Manufacturer → PriSteel market development → commission / representation fee. Vendimet komerciale mbeten njerëzore.</div></div><div class="pst-rep-home-stats">'+[[act.length,'Aktive'],[contacted.length,'Kontaktuar'],[meet.length,'Takime'],[represented.length,'Aktive']].map(function(x){return'<span class="pst-rep-home-stat"><b>'+x[0]+'</b><span>'+x[1]+'</span></span>'}).join('')+'</div></button>';
}
function injectSystemCard(){
 var grid=document.querySelector('#page-workspace-apps .pst-ws-appgrid');if(!grid||document.getElementById('pst-representations-system-card'))return false;
 var b=document.createElement('button');b.id='pst-representations-system-card';b.className='pst-ws-app';b.innerHTML='<div class="pst-ws-app-icon">◎</div><div class="pst-ws-app-name">Përfaqësime</div><div class="pst-ws-app-sub">Prodhues ndërkombëtarë, fit, kontakte dhe pipeline komercial</div>';b.onclick=open;grid.insertBefore(b,grid.firstChild);return true;
}
function open(){
 var p=ensurePage();hideOthers(p);p.style.display='block';p.classList.add('active');setRoute(true);try{window.scrollTo(0,0)}catch(e){}load(false);return true;
}
function back(){
...
function boot(){ensurePage();ensureHome();injectSystemCard();if(location.hash==='#perfaqesime')open();else load(false)}
...
new MutationObserver(function(){injectSystemCard()}).observe(document.documentElement,{childList:true,subtree:true});
