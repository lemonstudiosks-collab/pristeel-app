...
function canonicalWinnerName(v){var s=String(v||'').trim(),m=s.match(/^([A-Z]{2,3})_(.+)$/),p=['DE','DEU','GER','FR','FRA','EN','ENG','IT','ITA','NL','NLD','ES','SPA','PT','POR','PL','POL','CS','CZE','SK','SLK','HU','HUN','RO','RON','BG','BUL','DA','DAN','SV','SWE','FI','FIN','ET','EST','LV','LAV','LT','LIT','EL','GRE'];return m&&p.indexOf(m[1].toUpperCase())>=0?String(m[2]).trim():s;}
function names(r){var w=winner(r),a=uniq(arr(w.raw_names).concat(arr(w.names)).concat(w.name?[w.name]:[]).map(canonicalWinnerName));return a;}
...
function purposeLabel(v){return({procurement:'Prokurim',tender:'Tender / ofertim',sales:'Shitje',general:'Kontakt i përgjithshëm',person:'Person',contact_point:'Kontakt TED',company:'Faqja zyrtare',contact_form:'Formular zyrtar'})[v]||String(v||'Kontakt');}
function confidenceLabel(c,o){if(c&&c.draft_eligible===false)return'Jo për draft';if(o&&o.verified===true&&c&&c.confidence==='high')return'E verifikuar';if(o&&(o.domain||o.official_website)&&c&&c.confidence==='high')return'I atribuuar';return'Për verifikim';}
function contactLink(c){var type=String(c&&c.type||''),value=String(c&&c.value||'');if(type==='email')return'mailto:'+encodeURIComponent(value);if(type==='phone')return'tel:'+value.replace(/[^+0-9]/g,'');if(type==='website'||type==='contact_form')return safeHttp(value);return'';}
...
function contactRow(c,o){var href=contactLink(c),val=esc(c&&c.value||''),main=href?'<a href="'+esc(href)+'" '+(['website','contact_form'].indexOf(String(c.type))>=0?'target="_blank" rel="noopener"':'')+' style="font-weight:700;color:#29434E;text-decoration:none">'+val+'</a>':'<span style="font-weight:700;color:#29434E">'+val+'</span>';return'<div style="display:grid;grid-template-columns:minmax(180px,1.4fr) 105px 105px 70px;gap:9px;align-items:center;padding:8px 0;border-top:1px solid #EEF2F3;font-size:11px">'+main+'<span style="color:#66767D">'+esc(purposeLabel(c&&c.purpose))+'</span><span style="color:#66767D">'+esc(confidenceLabel(c,o))+'</span><span style="text-align:right;font-size:10px">'+sourceLink(c)+'</span></div>';}
function organizationCard(o){var cs=arr(o&&o.contacts),site=safeHttp(o&&o.official_website),warning=o&&o.verified===true?'':'<div style="margin-top:9px;padding:8px 10px;border-radius:8px;background:#FFF8EA;color:#80683F;font-size:9.5px">Kompania ose domain-i nuk është verifikuar plotësisht. Kontaktet kontrollohen para se të lejohen për draft.</div>';return'<section style="border:1px solid #DFE7EA;border-radius:12px;padding:13px 14px;margin-bottom:11px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><div style="font-size:13px;font-weight:800;color:#26353B">'+esc(o&&o.name||'Fitues')+'</div><div style="font-size:10px;color:#89959B;margin-top:2px">'+esc([o&&o.city,o&&o.country].filter(Boolean).join(' · '))+'</div></div>'+(site?'<a href="'+esc(site)+'" target="_blank" rel="noopener" class="pst-kek-btn">Web ↗</a>':'')+'</div>'+warning+(cs.length?'<div style="margin-top:9px">'+cs.map(function(c){return contactRow(c,o);}).join('')+'</div>':'<div style="font-size:11px;color:#8B969C;margin-top:10px">Nuk u gjet kontakt publik i verifikueshëm për këtë kompani.</div>')+'</section>';}
