...
function contactRow(c){var href=contactLink(c),val=esc(c&&c.value||''),main=href?'<a href="'+esc(href)+'" '+(['website','contact_form'].indexOf(String(c.type))>=0?'target="_blank" rel="noopener"':'')+' style="font-weight:700;color:#29434E;text-decoration:none">'+val+'</a>':'<span style="font-weight:700;color:#29434E">'+val+'</span>';return'<div style="display:grid;grid-template-columns:minmax(180px,1.4fr) 105px 105px 70px;gap:9px;align-items:center;padding:8px 0;border-top:1px solid #EEF2F3;font-size:11px">'+main+'<span style="color:#66767D">'+esc(purposeLabel(c&&c.purpose))+'</span><span style="color:#66767D">'+esc(confidenceLabel(c&&c.confidence))+'</span><span style="text-align:right;font-size:10px">'+sourceLink(c)+'</span></div>';}
